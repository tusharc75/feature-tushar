...
import { CustomDialogTransition, IMPORT_EXPORT_STATUS, IMPORT_EXPORT_TYPE, dateTimeFormat, gridLoadingTimeout, prepareDataForGrid } from 'src/constants/helpers';
...
const ImportExportDialog = ({ handleClose, type, resource, subResource, referenceId, handleExport, api, additionalParams, refresh }) => {
    ...
    const handleDownloadFile = (fileId) => {
        ...
        axiosInstance().get(`/import-export/download-file/${fileId}`, { responseType: 'arraybuffer' }).then((data) => {
    ...
    const uploadData = (event) => {
        if (event.target.files && event.target.files.length) {
            ...
            axiosInstance().post(importApi, formData, { responseType: 'blob', headers: { 'Content-Type': 'multipart/form-data' } })
                .then(() => {
                    toastConfig.setToastConfig({
                        open: true,
                        type: 'success',
