...
import { gridLoadingTimeout, isObjectEmpty, prepareDataForGrid, getLocalStorageArrayData, productCategory, RESOURCE_LABEL } from 'src/constants/helpers';
...
const AssignProductCategoryDialog = ({
    reference,
    referenceData = null,
    handleClose,
    handleSucess,
    ...
    const renderedFrom = `${routes.productCategory.title}_${reference}_selected`;
    ...
    const fetchData = () => {
        ...
        axiosInstance()
            .get(`${productCategory.api}${queryString}`)
            .then(({ data }) => {
    ...
    return (
        <Dialog fullWidth maxWidth="md" fullScreen={true} open={true} onClose={handleClose} aria-labelledby="assign-roles-dialog">
            <CustomDialogHeader
                title={`Add ${routes.productCategory.title}`}
...
export default AssignProductCategoryDialog;
