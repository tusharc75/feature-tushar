...
const AssignCompetenciesDialog = ({ onSuccess, handleClose, ids = [], isSubmitting = false, hideQty = false }) => {
  const renderedFrom = `assign_${camelCase(sidebarResource.competencies)}`;
  ...
  const fetchGridColumns = () => {
    axiosInstance()
      .get(`/field?resource=${sidebarResource.competencies}&view=true`)
      .then(({ data: { data } }) => {
        let newColumns = generateColumns(renderedFrom, data, routes.competenciesDetail.path, false);
        let cols = [...newColumns, ...getStaticFields()];
        if (hideQty) {
          setColumns([...cols]);
        } else {
  ...
  const fetchData = async (cancelTokenSource?: CancelTokenSource) => {
    try {
      ...
      const {
        data: { data }
      } = await axiosInstance().get(`/competencies${queryString}`, { cancelToken: cancelTokenSource?.token });
  ...
  return (
    ...
    >
      <CustomDialogHeader
        title={`Add ${resources?.competencies?.titlePlural}`}
...
export default React.memo(AssignCompetenciesDialog);
