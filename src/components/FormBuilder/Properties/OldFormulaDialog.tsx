...
const FunctionList = [{ name: 'IF', value: 'IF(,,)' }];
      ...
      >
        ...
        <CustomDialogContent>
          <Box padding={1}>
            ...
            <FormControlLabel
              control={<Checkbox name="required" checked={state.required} onChange={handleChecked} color="primary" />}
              label="Required"
            />
            <FormControlLabel
              control={<Checkbox name="isTooltip" checked={state.isTooltip} onChange={handleChecked} color="primary" />}
              label="Show Tooltip"
            />
            {state.isTooltip ? (
              <TextField
          ...
          <Box padding={1}>
            <Grid container spacing={5}>
              <Grid item xs={4}>
                ...
                <Box mt={1} mb={1} border={1} borderColor="var(--common-border-color)" minHeight={150} maxHeight={150} style={{ overflow: 'auto' }}>
                  {FunctionList.map((_function) => (
                    <ListItem
                      className={_function.value === functionSelect && classes.active}
                      key={_function.name}
                      dense
                      button
                      onClick={() => setFunctionSelect(_function.value)}
                    >
                      <ListItemText primary={_function.name} />
                    </ListItem>
                  ))}
                </Box>
                <Button variant="outlined" size="small" onClick={() => handleAddSyntax('function')} color="primary">
                  Insert
                </Button>
              </Grid>
              <Grid item xs={4}>
                    ...
                    _section.field.map(
                      (_field) =>
                        _field.type !== FieldList.FORMULA.type && (
                          <ListItem
                            className={(_field.fieldName ? _field.fieldName : camelCase(_field.fieldLabel)) === fieldSelect && classes.active}
                            key={_field._id}
                            dense
                            button
                            onClick={() => seFieldSelect(_field.fieldName ? _field.fieldName : camelCase(_field.fieldLabel))}
                          >
                            <ListItemText
                              primary={_field.fieldLabel + ' (' + (_field.fieldName ? _field.fieldName : camelCase(_field.fieldLabel)) + ')'}
                            />
                          </ListItem>
                        )
                    )
                ...
                <Button variant="outlined" size="small" onClick={() => handleAddSyntax('field')} color="primary">
                  Insert
                </Button>
              </Grid>
              <Grid item xs={4}>
                ...
                <Box mt={1} mb={1} border={1} borderColor="var(--common-border-color)" minHeight={150} maxHeight={150} style={{ overflow: 'auto' }}>
                  {OperatorList.map((_operator) => (
                    <ListItem
                      className={_operator.value === operatorSelect && classes.active}
                      key={_operator.name}
                      dense
                      button
                      onClick={() => setOperatorSelect(_operator.value)}
                    >
                      <ListItemText primary={_operator.value + '  ' + _operator.name} />
                    </ListItem>
                  ))}
                </Box>
                <Button variant="outlined" size="small" onClick={() => handleAddSyntax('operator')} color="primary">
                  Insert
                </Button>
              </Grid>
            </Grid>
