...
import { TActios, TInitialState } from '../hooks/tableReducer';
...
const ArrangeViewMenu = forwardRef<ApplyViewRef, ArrangeViewMenuProps>(
  ({ renderedFrom, dispatch, state, columns, hideSelection, expander, appliedView }, ref) => {
    ...
    const applyViewInTable = useCallback(
      ...
      [columns, dispatch, expander, gridMetaData, hideSelection, renderedFrom, setGridMetaData]
    );
    ...
    useEffect(() => {
      if (appliedView) {
        applyViewInTable(appliedView.order, appliedView.hide);
      } else if (defaultView) {
                          ...
                          return (
                            <Fragment key={d._id}>
                              ...
                              <ListItem
                                ...
                                key={d._id}
                                component={'li'}
                                selected={selected?._id === d._id && !appliedView}
                                onClick={() => {
                                  setAnchorEl(null);
                                  applyView(d);
                                }}
