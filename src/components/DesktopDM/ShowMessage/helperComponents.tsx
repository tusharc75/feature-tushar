...
export const RenderContent = ({
  ...
  return (
    <div
      className={cn(
        'leading-1.5 flex w-fit max-w-[320px] flex-col ',
        isReplying
          ? ''
          : `
    ...
    >
      <div className="flex items-center space-x-2 rtl:space-x-reverse">
        <span className="text-sm font-semibold text-gray-900 dark:text-white">{isUserMessage && isReplying ? 'You' : message.user.optionLabel}</span>
      ...
      {files.length > 0 &&
        (isReplying ? (
          ...
          <div className="mt-2.5">
            <FilePreview files={files} hasToDownload showDownloadButton />
          </div>
        ))}
    </div>
  );
