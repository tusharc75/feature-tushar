...
const SwipableListForMobile: FC<TSwipableListInputProps> = ({
  ...
  const actionField: any | null = React.useMemo(() => allColumns?.find((item) => item.accessor === 'action') || null, [allColumns]);

  const otherFields: any[] | null = React.useMemo(
    () => allColumns?.filter((item) => !item.primaryField && !['selection', 'action', 'expander'].includes(item.accessor) && item.isVisible) || null,
    [allColumns]
  );
  const expanderCol: any | null = React.useMemo(() => allColumns?.find((item) => item.accessor === 'expander') || allColumns[2], [allColumns]);
              ...
              <>
                {footerGroups.map((group, index) => (
                  <div key={index} className="flex justify-between [border-top:1px_solid_var(--common-border-color)] pt-1 items-center mt-4 px-2">
                    <h6 className="text-[14px]">{group?.headers?.find((g) => g.accessor === 'index')?.render('Footer')}</h6>
                    {group.headers.map((column) => {
                      if (column.Footer.name === 'emptyRenderer2' || column.Footer.name !== 'Footer' || column.accessor === 'index') return null;
                      return (
                        <div key={column.accessor} className="text-truncate font-weight-bold text-black flex flex-col items-center">
...
const RenderCellWithHeader = ({ field, row, submitInput, handleCellClick, handleKeyDown, cellValue, setCellValue, state, dispatch }: any) => {
  ...
  const cell = row.cells.find((cell: any) => cell?.column?.accessor === field?.accessor);
  if (!cell) return null;
