    ...
    return (dates && calType === calendarType) && <Box height={54} display="flex" width={totalDay * dayPixel} bgcolor="grey.200" style={{ position: "sticky", top: 0, zIndex: 2, }} >
        {calendarType === "week" ?
            Array.from(dates, (date, index) => {
                return <Box key={index} borderColor="grey.300" minWidth={(moment(date).daysInMonth() * dayPixel)}>
        ...
        {calendarType === "quater" ?
            dates.map((date, index) => (
                <Box key={index} p={2} display="inline" minWidth={dayPixel * parseInt(date.days)} border={1} borderColor="grey.300">
