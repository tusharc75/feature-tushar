...
export default function Calander({ calendarType, dayPixel, startDate, endDate }) {
    ...
    return (dates && calType === calendarType) && <Box height={54} display="flex" width={totalDay * dayPixel} bgcolor="grey.200" style={{ position: "sticky", top: 0, zIndex: 2, }} >
        {calendarType === "week" ?
            Array.from(dates, (date, index) => {
                return <Box key={index} align="center" borderColor="grey.300" minWidth={(moment(date).daysInMonth() * dayPixel)}>
                    <Typography variant="caption" display="block">{moment(date).format("MMM YYYY").toUpperCase()}</Typography>
                    {Array.from(Array(moment(date).daysInMonth()), (data, index) => {
                        return <div key={index} style={{ minWidth: dayPixel, maxWidth: dayPixel, textAlign: "center", float: "left" }}>
                            <Typography variant="caption" display="block">{moment(moment(date).format("YYYY-MM") + "-" + (index + 1)).format("ddd")[0]}</Typography>
                            <Typography variant="caption" display="block">{moment(moment(date).format("YYYY-MM") + "-" + (index + 1)).format("DD")}</Typography>
                        </div>
                    })}
                </Box>
            }) : null}
        {calendarType === "month" ?
            Array.from(dates, (date, index) => {
                return <Box key={index} p={2} display="inline" minWidth={moment(date).daysInMonth() * dayPixel}
                    align="center" border={1} borderColor="grey.300"  >
                    <Typography variant="caption" display="block">{moment(date).format("MMM YYYY").toUpperCase()}</Typography>
                </Box>
            }) : null}
        {calendarType === "quater" ?
            dates.map((date, index) => (
                <Box key={index} p={2} display="inline" minWidth={dayPixel * parseInt(date.days)} align="center" border={1} borderColor="grey.300">
                    <Typography variant="caption" display="block">{(moment(date.q_s_date).format("MMM") + "-" + moment(date.q_e_date).format("MMM") + " " + moment(date.q_s_date).format("YYYY")).toUpperCase()}</Typography>
                </Box>
            ))
            : null}
    </Box>
