...
const AddExistingProduct = (props) => {
  ...
  useEffect(() => {
    if (isProductTemplate) {
      if (productCategory && productCategory !== '') {
        axiosInstance()
          .post(`/product-template/template/` + productCategory, { entity: [] })
          .then(({ data: { data } }) => {
  ...
  const handleAdd = () => {
    ...
    let rows = productList.filter((val) => selectedRecords.filter((u) => val._id === u._id).length > 0);
    rows.forEach((_d) => {
      _d.productId = _d._id;
      if (_d.fields) {
      ...
      <div className="listing-grid p-3">
        <Box mb={2}>
          <h6 className="form-label-style mt-0 mb-0" style={{ borderBottom: 'none' }}>
            * Select product using checkbox and click Add button for add product
          </h6>
          <Grid container>
