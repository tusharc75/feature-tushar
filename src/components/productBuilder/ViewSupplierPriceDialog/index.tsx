...
const ViewSupplierPriceDialog = (props) => {
  ...
  const fetchProductGridData = () => {
    axiosInstance()
      .get(`/quote-builder/quote-product-supplier-response/${quoteData?._id}/${productBuilderId}`)
      .then(({ data: { data } }) => {
  ...
  const handleAdd = (requestId) => {
    axiosInstance()
      .put(`/quote-builder/apply-bulk-supplier-price`, { requestId: requestId })
      .then(({ data: { data } }) => {
        onSuccess();
        fetchProductGridData();
      })
  ...
  const handleReject = (content) => {
    axiosInstance()
      .put(`/quote-builder/apply-reject/${rejectId}`, { body: content ? content : '', protected: true })
      .then(({ data }) => {
                      ...
                      <Box display="flex">
                        ...
                        <Box padding="5px">
                          <Typography variant="subtitle2">
                            {data?.supplierAccount?.optionLabel && `Supplier Account : ${data?.supplierAccount?.optionLabel}`}&nbsp;&nbsp;&nbsp;&nbsp;{' '}
