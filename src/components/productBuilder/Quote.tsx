...
const Quote = (props) => {
    ...
    return (<div>
        {newVersion ?
            (
                <Button variant="contained" size="small" startIcon={<FaRegMoneyBillAlt />} color="primary" onClick={() => { CreateorOpenQuoteBuilder() }}>Generate Quote</Button>
            ) : (

                <Tooltip title={message}>
                    <Link to={'/quote-builder/' + productBuilderId}>
                        <Button variant="contained" size="small" startIcon={<BsEye  size={15}/>} color="primary" onClick={() => { }}>View Quote</Button>
                    </Link>
                </Tooltip>
            )}
    </div>
