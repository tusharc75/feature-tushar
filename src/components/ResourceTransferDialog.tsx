...
export default function ResourceTransferDialog(props) {
  ...
  const handleDeleteResource = () => {
    let deleteId = Array.isArray(fromResource)
      ? fromResource.map((d) => d._id)
      : fromResource?._id
        ? fromResource?._id
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <Box p={2}>
          <Typography>{`First you have to transfer all the resources related to this to another ${resource}`}</Typography>
        </Box>
        <Box p={2}>
          <Autocomplete
            size="small"
