...
export const createDmsUrl = () => {
  const token = localStorage.getItem('token');
  let redirectUrl: string = VITE_APP_DMS_URL;
  if (token) {
    redirectUrl = `${VITE_APP_DMS_URL}/oauth?token=${encodeURIComponent(token)}&redirect=/document`;
  }
  return redirectUrl;
};
...
const DMSRedirect = () => {
  ...
  return VITE_APP_DMS_URL &&
    ((user?.user?.brand === '66c81f4780d6ec9b112302ef' && VITE_APP_ENV === 'production') || VITE_APP_ENV !== 'production') ? (
