...
const ArrangeView = ({
  ...
  const applyViewInTable = (order: string[], hide: string[], sizes: { [key: string]: number } | null, name?: string, id?: string) => {
    ...
    dispatchTable?.({ type: 'setVisibleColumns', visibleColumns: columnHiddenStateData });
    dispatchTable?.({ type: 'setColumnOrder', columnOrder: columnOrder });
    dispatchTable?.({ type: 'setColumnSizes', sizes: sizes });
    setOrderAndVisibility?.({ order: columnOrder, visible: columnHiddenStateData });
