...
const SupportTicket = () => {
    ...
    const fetchSupportTicketData = () => {
      ...
      axiosInstance().get(`/support-ticket${queryString}`)
        .then(({ data: { data } }) => {
    ...
    const NameRenderer = (params) => {
      return <span className=" d-flex gap-2 align-items-center">
        <Link className="link" to={`${routes.supportTicketDetail.path}/${params.data._id}`}>
          {params.value}
        </Link>
      </span>
    };
            ...
            <Grid container className={styles.filter_side_container}>
              <Grid item xs={12} md={6} sm={12} className={isMobile ? styles.mobile_panel : 'd-flex align-items-center gap-1'}>
                <div className="d-flex align-items-center">
                  <ImTicket size={20} style={{ paddingBottom: '3px' }} />
                  <span className="listingHeader">{routes.supportTicket.title}</span>
                </div>
              </Grid>
