...
const OrgChart = ({ positions, getOrgChart, chartId, update = null, updateChart = null, edit = null, onClickNode, google }) => {
  ...
  const drop = (event) => {
    ...
    if (dropNode && draggedNode) {
      if (draggedNode.id !== dropNode.id && draggedNode.id !== dropNode.parentId) {
        if (updateChart) {
          updateChart(draggedNode, dropNode);
        }
      }
    }
  };
  ...
  const drawChart = () => {
    ...
    if (orgChartDiv) {
      ...
      google.visualization.events.addOneTimeListener(chart, 'ready', () => {
        const nodes = window.document.getElementsByClassName('google-visualization-orgchart-node');
      ...
      let orgPositions = orgChart.map((p) => [{ v: `${p.id}`, f: template(p) }, !p.parentId || p.parentId === '0' ? null : `${p.parentId}`, p.title]);
  ...
  return <div id={chartId} style={{ overflow: 'auto' }}></div>;
};
