...
const CreateProduct = (props) => {
    ...
    axiosInstance()
      ...
      .then(({ data: { data } }) => {
        ...
        if (productId) {
            ...
            .then(({ data: { data } }) => {
              ...
              newField.map((_f) => {
                if (_f.fieldName === 'currency') {
                  setCurrencySymbol(getUniqueCurrencies().find((d) => d.currencyCode === data.productData['currency'])?.symbolNative);
                }
              });
              setInitialData({
                fields: newField,
          ...
          _fields.some((_f) => {
            if (_f.fieldName == 'currency') {
              setCurrencySymbol(getUniqueCurrencies().find((d) => d.currencyCode === user?.user?.currency)?.symbolNative);
        ...
        if (_fields.length > 0) {
          const productCategoryDropdownData = _fields.find((d) => d.fieldName === 'productCategory');
          if (productCategoryDropdownData) {
            if (!productId) {
              setProductCategoryDataSource(productCategoryDropdownData.option);
            } else {
              let currentContactRemovedDataSource = productCategoryDropdownData.option.filter((d) => d?.optionValue !== newProductCategoryId);
  ...
  const handleChangeCategory = (value, label, isChange, entity) => {
    if (value && value !== '') {
      ...
      axiosInstance()
        .post(`/product-template/template/` + value, { entity: entity })
        .then(({ data: { data } }) => {
          ...
          if (isChange) {
            ...
            if (defaultproductTemplate !== '') {
              axiosInstance()
                .get(`/product-template/fields/` + defaultproductTemplate)
                .then(({ data: { data } }) => {
                  ...
                  axiosInstance()
                    .get(`/price-template/product-template/` + defaultproductTemplate)
                    .then(({ data: { data } }) => {
  ...
  const handleChangeProductTemplate = (value) => {
    if (value && value !== '') {
      ...
      if (result.length) {
        axiosInstance()
          .get(`/product-template/fields/` + result[0].optionValue)
          .then(({ data: { data } }) => {
            let newField = [...masterFields, ...fields];
            data.fields.forEach((_f) => {
              newField.push(_f);
              ...
              .then(({ data: { data } }) => {
                ...
                setInitialData({
                  ...
                  values: {
                    ...getObjKeys('', newField),
                    ...ref.current.values,
                    productTemplate: result[0].optionValue,
                    priceTemplate: defaultpriceTemplate
                  }
                });
                EvaluteproductFields(newField);
                        ...
                        <div key={i}>
                          <div className={'detail-box-content detail-product-box'}>
                            ...
                            <IconButton
                              ...
                              color="primary"
                              size="small"
                              onClick={(e) => handleOpenAddField(e, section.name)}
                            >
                                        ...
                                        <Grid container spacing={1}>
                                          ...
                                          >
                                            <FormTypes
                                              ...
                                              onChange={(e, val) => {
                                                ...
                                                if (isProductTemplate) {
                                                  handleChangeCategory(
                                                    val && val.optionValue ? val.optionValue : '',
                                                    val && val.optionLabel ? val.optionLabel : '',
                                                    true,
                                                    null
                                                  );
                                                }
                                              ...
                                              size="small"
                                              values={
                                                newProductCategoryId ? initializeProductCategoryDropdown(values, productCategoryDataSource) : values
                                              }
