...
const ProductTemplateSchema = object().shape({
  name: string().min(3, 'Too Short!').max(50, 'Too Long').required('Template name is required')
});
...
const ProductTemplate = () => {
  ...
  const fetchOneProductTemplate = () => {
    if (id === '0') {
      ...
      axiosInstance()
        .get(`/product-template/default-field`)
        .then(({ data: { data } }) => {
          const _data = [];
  ...
  const handleSave = (values) => {
    ...
    data.generalRemark = generalRemarkData;
            ...
            <Grid container spacing={2}>
              <Box className={classes.tinyMCEContainer}>
                <TinyMce
                  id="generalRemark"
                  onChange={(value) => {
                    setGeneralRemarkData(value);
                  }}
                  width={'100%'}
                  height={300}
                  initialValue={generalRemarkData}
