...
import { getObjKeysWithValues, getObjKeys, yupSchema, pricingCondition, CustomDialogTransition, getUniqueCurrencies, productTemplate, gridLoadingTimeout, prepareDataForGrid } from "../../../constants/helpers";
import CommonSkeleton from '../../../components/Helpers/CommonSkeleton'
import {
    Box, Grid, TextField, InputAdornment, Chip, Badge,
    Select, FormControl, InputLabel, IconButton, Tabs, Tab
...
const ConditionDialog = ({ pricingConditionId, conditionData, handleClose, handleSuccess, detailData, isBulkedit }) => {
    ...
    const fetchProductFields = () => {
        if (conditionData?.productDetail?.productTemplate) {
            axiosInstance().get(`${productTemplate.productTemplateApi}/fields/${conditionData?.productDetail?.productTemplate}`).then(({ data: { data } }) => {
    ...
    const fetchProducts = () => {
        ...
        axiosInstance().get(`${pricingCondition.api}/${pricingConditionId}/rate/${conditionData?.materialId}`).then(({ data: { data } }) => {
                ...
                removeRentDialog.open && (
                    <ConfirmationDialog
                        ...
                        onOk={() => {
                            ...
                            axiosInstance().post(`${pricingCondition.api}/${pricingConditionId}/rate/${conditionData?.materialId}/remove`, { ids: removeRentDialog.ids }).then(() => {
