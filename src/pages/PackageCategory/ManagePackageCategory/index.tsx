...
export const ManagePackageCategory = ({ isClone = false, packageCategoryId = null, isRedirectToDetailPage = true, onClose, onSuccess }) => {
  ...
  const handleSubmit = (values) => {
    ...
    if (packageCategoryId && isClone === false) {
      values._id = packageCategoryId;
      axiosInstance().put(`${packageCategory.api}`, values).then(({ data }) => {
      ...
      axiosInstance().post(`${packageCategory.api}`, values).then(({ data: { data, message } }) => {
        ...
        if (isRedirectToDetailPage) {
          history.push(`${routes.packageCategoryDetail.path}/${data._id}`);
        }
        onSuccess(data);
        setIsSubmitting(false);
      }).catch((error) => {
          ...
          {({ values, errors, touched, setFieldValue, submitForm }) => (
            <>
              <CustomDialogHeader
                title={
                  !packageCategoryId
                    ? `Create ${resources?.packageCategory?.titleSingular}`
                    : `${isClone ? `Clone - ${cloneHeading}` : `Update ${initialData?.values?.packageCategory}`}`
                }
                onClose={() => {
