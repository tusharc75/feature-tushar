...
const ManageRepairType = ({ isClone = false, repairTypeId = null, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        axiosInstance().get(`/field?resource=${repairType.resource}`).then(({ data: { data } }) => {
        ...
        {formsData && formsData.length ?
            ...
            >
                {({ values,
                    ...
                    <Fragment>
                        <CustomDialogHeader title={repairTypeId ? (isClone ? "Clone" : `Update ${repairTypeData?.repairType}`) : "Create " + routes.repairType.title}
                            onClose={() => {
