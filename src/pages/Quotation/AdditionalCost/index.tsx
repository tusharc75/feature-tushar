...
const AdditionalCost = ({ quotationData, setNextStep, renderedFrom }) => {
    ...
    const fetchAdditionalCost = async () => {
        try {
            ...
            var data: any = []
            const response = await axiosInstance().get(`${quotation.api}/additionalcost/${quotationData._id}`)
            data = response?.data?.data
    ...
    const handleAddCost = (rows) => {
        axiosInstance().post(`${quotation.api}/additionalcost/${quotationData._id}/add`, { additionalCost: rows })
            .then(() => {
                fetchAdditionalCost()
    ...
    const handleUpdateCost = (rows) => {
        axiosInstance().put(`${quotation.api}/additionalcost/${quotationData._id}/update`, { additionalCost: rows })
            .then(() => {
                fetchAdditionalCost()
                setShowCostDialog(false)
            }).catch((error) => {
    ...
    const handleDeleteCost = (ids) => {
        axiosInstance().post(`${quotation.api}/additionalcost/${quotationData._id}/delete`, { ids })
            .then(() => {
                fetchAdditionalCost()
            }).catch((error) => {
                toastConfig.setToastConfig(error)
