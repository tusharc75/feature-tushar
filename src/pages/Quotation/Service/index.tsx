...
const Product = ({ quotationData, setNextStep, renderedFrom, stepFullScreen }) => {
    ...
    const fetchQuotationService = () => {
        ...
        axiosInstance().get(`${quotation.api}/service/${quotationData._id}`).then(({ data: { data } }) => {
    ...
    const handleAddService = (rows) => {
        axiosInstance().post(`${quotation.api}/service/${quotationData._id}/add`, { services: rows })
            .then(() => {
                fetchQuotationService()
    ...
    const handleUpdateService = (rows) => {
        axiosInstance().put(`${quotation.api}/service/${quotationData._id}/update`, { services: rows })
            .then(() => {
                fetchQuotationService()
                setShowServiceDialog(false)
            }).catch((error) => {
    ...
    const handleDelete = () => {
        axiosInstance().post(`${quotation.api}/service/${quotationData._id}/delete`, { ids: deleteQuotationService })
            .then(() => {
                fetchQuotationService()
        ...
        <Fragment>
            <Box display="flex" justifyContent="space-between" m={1}>
                ...
                <div className="d-flex gap-2">
                    ...
                    >
                        {permissions?.quotation?.isDelete && <MenuItem onClick={() => {
                            closeActions()
                            setShowDeleteConfirmBox(true)
