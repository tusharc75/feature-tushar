...
const Address = () => {
  ...
  const handleDelete = () => {
    ...
    axiosInstance()
      .put(`${routes?.address.path}/remove`, { ids: ids })
      .then(() => {
  ...
  return (
    <section className="main-container-v1">
      <div className="headerbox-v1">
        <CustomBreadCrumbs routes={[{ ...routes.address, title: resources?.address?.titlePlural }]} />
        <ImportExportLinks
          permissions={permissions?.address}
          module={resources?.address?.titlePlural}
          api={routes?.address.path}
      ...
      <CustomContainer>
        <ListingPageHeader
          ...
          isActionButtonVisible={permissions?.address?.isDelete}
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete ${deleteRecord ? `${resources?.address?.titleSingular?.toLowerCase()} :
            ${deleteRecord?.fullAddress}` : `selected ${resources?.address?.titlePlural?.toLowerCase()}`} ?`}
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
          }}
