...
const CompetencyType = () => {
  ...
  const fetchData = async (cancelTokenSource?: CancelTokenSource) => {
    ...
    axiosInstance()
      .get(`${routes?.competencyType.path}${queryString}`, { cancelToken: cancelTokenSource?.token })
      .then(({ data: { data } }) => {
    ...
    <section className="main-container-v1">
      ...
      <CustomContainer>
        <ListingPageHeader
          ...
          isActionButtonVisible={permissions?.competencyType?.isDelete}
          actionButtonProps={{ disabled: selectedRecords?.length ? false : true }}
          actionMenuItems={<ActionMenuItems />}
          addButtonOnclick={() => {
            setShowManageDialog({ open: true, isClone: false, idToClone: null });
          }}
          isAddButtonVisible={permissions?.competencyType?.isCreate}
        />
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete ${deleteRecord ? `${resources?.competencyType?.titleSingular?.toLowerCase()} : ${deleteRecord?.competencyType || ''}` : `selected ${resources?.competencyType?.titlePlural?.toLowerCase()}`} ?`}
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
          }}
