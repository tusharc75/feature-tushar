...
const NavBar = ({ state }: { state: UseUsermanual }) => {
  const [themeColor, toggleThemeColor] = useAppTheme();
  const [isMobile] = useUserManualStore((prev) => prev.isMobile);
  const [toggleSidebar, setStore] = useUserManualStore((prev) => prev.toggleSidebar);
