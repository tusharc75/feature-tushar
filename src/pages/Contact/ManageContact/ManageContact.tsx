...
export default function ManageContact(props) {
  ...
  useEffect(() => {
    if (contactData.fields.length > 0) {
      ...
      if (fromProject) {
        ...
        const ownerCollaboratorDropdownData = contactData.fields.filter((d) => ['owner', 'collaborator'].indexOf(d.fieldName) !== -1);
  ...
  const initializeAccountDropdown = (values, accountSource) => {
    if (values && values.hasOwnProperty('accountName')) {
      const getNewAddedAccount = accountSource.find((d) => d?.optionValue === accountId);
                                    ...
                                    <Grid key={field.fieldName} item xs={12} sm={6} md={6}>
                                      {field.fieldName === 'owner' ? (
                                        <FormTypes
                                          ...
                                          onOpen={() =>
                                            // !fromProject &&
                                            onOwnerDropdownOpen(values.collaborator, values.entity ? values.entity : [])
                                          }
                                        />
                                        ...
                                        <FormTypes
                                          ...
                                          onOpen={() =>
                                            // !fromProject &&
                                            onCollaboratorOwnerMultiselectOpen(values.owner, values.entity ? values.entity : [])
                                          }
                                        />
                                            ...
                                            <Grid item xs={1} sm={1} md={1}>
                                              <Tooltip title="Create Account" className={`${classes.createAccountTooltip} mt-1`}>
                                                ...
                                                >
                                                  <AddIcon
                                                    color={
                                                      fromProject || (!isNew && field.disableOnEdit) || (accountId && isAccountFieldDisable)
                                                        ? 'disabled'
                                                        : 'primary'
                                                    }
                                                  />
                                        ...
                                        <Grid container spacing={1}>
                                          ...
                                          {permissions[accountResource].isCreate && (
                                            <Grid item xs={1} sm={1} md={1}>
                                              <Tooltip title="Create Reports To" className={`${classes.createAccountTooltip} mt-1`}>
                                                <IconButton
