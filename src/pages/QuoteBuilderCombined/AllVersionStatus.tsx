...
export default function AllVersionStatus({ open, onClose, quoteId, quoteData, quotePermissions, fetchQuoteData, handleChangeVersionFromAllVersion, handleCloneQuoteWithVersionFromAllVersion }) {
    ...
    const columns = [
        {
            accessor: "versionNumber", Header: "Version", show: true, width: 140, disabled: true, Cell: ({ row }) => (<div className="link"
                onClick={() => {
        ...
        },
        {
            ...
            Cell: ({ row }) => (<div className="link text-truncate"
                onClick={() => {
                    fetchQuoteData(row?.original?.versionNumber);
                    handleChangeVersionFromAllVersion(row?.original?.versionNumber);
                }}>
        ...
        {
            accessor: "processStatus", Header: "Current Step", show: true,
            Cell: ({ row }) => (<span className="link text-truncate"
                onClick={() => {
                    fetchQuoteData(row?.original?.versionNumber);
                    handleChangeVersionFromAllVersion(row?.original?.versionNumber);
                }}>
        ...
        {
            accessor: "comment", Header: "Comment", show: true,
            Cell: ({ row }) => (row?.original?.comment ? <div>
                <p>{row?.original?.comment}</p>
            </div> : <NoDataCell />)
        },
        {
            accessor: "totalcost", Header: "Total Cost", show: true,
            Cell: ({ row }) => (row?.original?.totalcost ? <div>
                <p>{row?.original?.totalcost}</p>
            </div> : <NoDataCell />)
        },
        {
            accessor: "totalSalesPrice", Header: "Total Sales Price", show: true,
            Cell: ({ row }) => (row?.original?.totalSalesPrice ? <div>
