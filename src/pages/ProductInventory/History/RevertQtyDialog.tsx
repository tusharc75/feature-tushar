...
import { CustomDialogTransition, RESOURCE_LABEL, productInventory } from 'src/constants/helpers';
...
function RevertQtyDialog({ referenceType, productName, product, onClose, onSuccess, qty, revertedQty, ledgerId }) {
  ...
  const handleSubmit = (values) => {
    ...
    if (referenceType === "workOrder") {
      axiosInstance().put(`/material-handling/revert/${ledgerId}`, {...data,referenceType:RESOURCE_LABEL.workOrder})
        .then(({ data: { data } }) => {
          setLoading(false);
          onSuccess();
        })
