...
// const instanceIdMap = new WeakMap<HTMLElement, string>(); // rootElement -> instanceId
// const instanceState = new Map<string, InternalState>(); // instanceId -> state
...
//         if (!['Tab', 'Escape'].includes((e as KeyboardEvent).key)) {
//             e.stopPropagation();
//         }
//     });
// };
...
// const makeInstanceId = (schema: MyGridSchema) => {
//     if (schema.id && typeof schema.id === 'string' && schema.id.trim()) return schema.id;
//     // fallback random id (short)
//     return `${schema.type}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
// };

// const getInstanceIdForRoot = (root: HTMLElement, schema: MyGridSchema) => {
//     // Priority:
//     // 1) schema.id (explicit stable id)
//     // 2) dataset.pluginInstanceId saved previously on root
//     // 3) create new id, save to root.dataset and instanceIdMap
//     if (schema.id && typeof schema.id === 'string' && schema.id.trim()) {
//         return schema.id;
//     }
//     const existing = instanceIdMap.get(root) || root.dataset.pluginInstanceId;
//     if (existing) {
//         instanceIdMap.set(root, existing);
//         root.dataset.pluginInstanceId = existing;
//         return existing;
//     }
//     const newId = makeInstanceId(schema);
//     instanceIdMap.set(root, newId);
//     root.dataset.pluginInstanceId = newId;
//     return newId;
// };

// const focusCellIfNeeded = (root: HTMLElement, instanceId: string) => {
//     const s = instanceState.get(instanceId);
...
// const persistStateDebounced = (state: InternalState, schema: MyGridSchema, onChange?: UIRenderProps<MyGridSchema>['onChange']) => {
//     if (!onChange) return;
//     if (state._saveTimer) {
...
//             {
//                 key: 'content',
//                 value: JSON.stringify(state.rows.map((row, i) => row.concat(`Row ${i + 1}`))),
//               },
...
// const persistStateImmediate = (state: InternalState, schema: MyGridSchema, onChange?: UIRenderProps<MyGridSchema>['onChange']) => {
//     if (!onChange) return;
//     if (state._saveTimer) {
//         window.clearTimeout(state._saveTimer);
//         state._saveTimer = null;
...
//         const instanceId = getInstanceIdForRoot(rootElement, schema);
//         // state for this plugin instance (persisted globally by instanceId)
//         let state = instanceState.get(instanceId);
//         const incomingRows = (schema?.rows as string[][]) ?? [['']];
//         const incomingCols = (schema?.cols as number) ?? 1;

//         if (!state) {
//             state = { rows: JSON.parse(JSON.stringify(incomingRows)), cols: incomingCols, focusedId: null, _saveTimer: null };
//             instanceState.set(instanceId, state);
//         } else {
//             // if parent schema changed externally, reconcile
...
//         // capture focused cell only if focus is inside this root
//         const active = document.activeElement as HTMLElement | null;
//         if (active && active.dataset && (active.dataset as any).cellId && rootElement.contains(active)) {
//             state.focusedId = (active.dataset as any).cellId;
//         }
...
//             const totalHeight = rootElement.clientHeight || (schema?.height ?? state!.rowHeights!.reduce((a, b) => a + b, 0));
//             const borderSum = (state!.rows.length - 1) * (typeof schema?.colBorderWidth === 'number' ? schema.colBorderWidth : 1);
//             const totalFixedHeight = Math.max(MIN_ROW_PX * state!.rows.length, totalHeight - borderSum);
...
//                     state!.focusedId = cellId;
//                     const val = (e.target as HTMLTextAreaElement).value;
//                     state!.rows[rowIndex][colIndex] = val;
//                     state!.contentChanged = true;
//                     persistStateDebounced(state!, schema, onChange);
//                 };
//                 const onBlur = () => {
//                     persistStateImmediate(state!, schema, onChange);
...
//                     const { selectionStart = 0, selectionEnd = 0 } = input as any;
...
//                     state!.contentChanged = true;
//                     persistStateDebounced(state!, schema, onChange);
//                     const newCursorPos = selectionStart + (textToInsert?.length ?? 0);
//                     input.focus();
...
//         // re-apply sizes and restore focus
//         setTimeout(() => {
//             applyColWidthsToRows();
//             applyRowHeights();
//             createColResizers();
//             createRowResizers();
//             setTimeout(() => focusCellIfNeeded(rootElement, instanceId), 0);
//         }, 0);
//     },

//     pdf: async ({ page, schema }: PDFRenderProps<MyGridSchema>) => {
//         // keep your existing pdf rendering code (same as before)
...
//     propPanel: {
//         schema: () => ({
//             cols: { title: 'Columns', type: 'number', min: 1, max: 20 },
//             tableBorderWidth: { title: 'Table Border Width', type: 'number', min: 0, max: 10, step: 0.5 },
//             tableBorderColor: { title: 'Table Border Color', type: 'string', widget: 'color' },
//             colBorderWidth: { title: 'Cell Border Width', type: 'number', min: 0, max: 5, step: 0.5 },
//             colBorderColor: { title: 'Cell Border Color', type: 'string', widget: 'color' },
//             textSize: { title: 'Text Size', type: 'number', min: 6, max: 24 },
//             textColor: { title: 'Text Color', type: 'string', widget: 'color' },
//             textWeight: { title: 'Text Weight', type: 'string', enum: ['normal', 'bold'] }
//         }),
//         defaultSchema: {
//             id: 'myGrid_default_0',
//             name: 'Grid',
