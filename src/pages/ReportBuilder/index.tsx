...
const ReportBuilder = () => {
    ...
    const columns = [
      {
        ...
        Cell: ({ row }) => (
          <div>
            <Link className="link" to={`${routes.reportBuilderDetail.path}/${row?.original?._id}`} title={row?.original?.name}>
              {row?.original?.name}
            </Link>
          </div>
        )
  ...
  const fetchData = async (cancelTokenSource?: CancelTokenSource) => {
    ...
    axiosInstance()
      .get(`/report-builder${queryString}`, { cancelToken: cancelTokenSource?.token })
      .then(({ data: { data, count } }) => {
        let rows = data?.map((u) => {
          let finalObject = prepareDataForGrid(u, user);
          finalObject['isChecked'] = selectedRecords.some((s) => s._id === u._id);
          finalObject['allowedToEdit'] = permissions?.reportBuilder?.isUpdate;
          finalObject['canDelete'] = permissions?.reportBuilder.isDelete;
          return finalObject;
        });
  ...
  const handleDelete = () => {
    ...
    axiosInstance()
      .put(`/report-builder/remove`, { ids: ids })
      .then(() => {
  ...
  return (
    <section className="main-container-v1">
      <div className="headerbox-v1">
        <CustomBreadCrumbs routes={[{ ...routes.reportBuilder, title: resources?.reportBuilder?.titlePlural }]} />
      </div>
      <CustomContainer>
        <ListingPageHeader
          searchValue={search}
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete ${deleteRecord ? `report : ${deleteRecord?.name || ''}` : `selected report(s) ?`}`}
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
          }}
