...
const ManageTriggerNotificationMaster = ({ onClose, onSuccess, isClone = false, id = null }) => {
  ...
  const fetchFields = async () => {
    try {
      ...
      if (id) {
        axiosInstance()
          .get(`${routes?.triggerNotificationMaster?.path}/${id}`)
          .then(({ data: { data } }) => {
  ...
  const handleSubmit = (values) => {
    ...
    if (id && !isClone) {
      values._id = id;
      axiosInstance()
        .put(`${routes.triggerNotificationMaster?.path}`, values)
        .then(({ data }) => {
          setSubmitting(false);
          onSuccess();
      ...
      axiosInstance()
        .post(`${routes.triggerNotificationMaster?.path}`, values)
        .then(({ data }) => {
          setLoading(false);
          setSubmitting(true);
          history.push(`${routes.triggerNotificationMasterDetail.path}/${data?.data?._id}`);
          toastConfig.setToastConfig({
            open: true,
            type: 'success',
            message: data.message
...
export default ManageTriggerNotificationMaster;
