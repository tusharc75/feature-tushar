...
const ManageStorageLocation = ({ isClone = false, storageLocationId = null, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        axiosInstance().get(`/field?resource=${storageLocation.resource}`).then(({ data: { data } }) => {
            let fieldsDataForCreate = data.filter((obj) => obj.isCreate).map((d: any) => d.fieldData);
            let fieldsDataForUpdate = data.filter((obj) => obj.isUpdate).map((d: any) => d.fieldData);
            if (storageLocationId) {
                axiosInstance().get(`${storageLocation.api}/` + storageLocationId).then(({ data: { data } }) => {
    ...
    const handleSubmit = (values) => {
        ...
        if (storageLocationId && isClone === false) {
            ...
            axiosInstance().put(`${storageLocation.api}`, values).then(({ data }) => {
            ...
            axiosInstance().post(`${storageLocation.api}`, values).then(({ data }) => {
                toastConfig.setToastConfig({
                    open: true,
                    type: 'success',
                    message: data.message
