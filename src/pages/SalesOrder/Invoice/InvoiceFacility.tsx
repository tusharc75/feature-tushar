...
const InvoiceFacility = ({ salesOrderData }) => {
  ...
  const handlePDF = (type, PDFType) => {
    ...
    axiosInstance()
      .get(
        PDFType === 'Detail' ? `${routes.salesOrder.path}/${salesOrderData._id}/pdf/detail` : `${routes.salesOrder.path}/${salesOrderData._id}/pdf`
      )
      .then(({ data }) => {
        axiosInstance()
          ...
          .then(({ data }) => {
            if (type === 'Download') {
              ...
              link.href = url;
              link.setAttribute('download', `Sales-Order-Invoice${salesOrderData.salesOrderNo}.pdf`);
