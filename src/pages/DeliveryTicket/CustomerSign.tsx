...
import { Grid, Box, Button, Typography, Paper, makeStyles } from "@material-ui/core";
...
import { deliveryTicket } from "../../constants/helpers";
...
const CustomerSign = () => {
    ...
    const fetchDeliveryTicketData = async () => {
        setLoading(true);
        axios.get(backendApi + `/delivery-ticket/get-delivery-by-id/${id}`)
            .then(async ({ data }) => {
                setLoading(false);
    ...
    const handleSignature = (signedData) => {
        ...
        axios.put(backendApi + `/delivery-ticket/update-customer-signature`, {
            deliveryTicketId: id,
                            ...
                            {!deliveryTicketData ? (
                                ...
                                >
                                    <span className="d-flex align-items-center">
                                        <span className="listingHeader"> {deliveryTicketData?.signatures?.length === 4 ?
                                            "Please Sign-in "
                                            : "Customer has been successfully signed in"}</span>
                                    </span>
                                </Typography>
                            )}
                            <Box>
