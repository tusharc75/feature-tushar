		...
		let coloum: any = [
			...
			},
			{
				...
				Cell: ({ row }) => row.original['type'] ? <div>
					<p className="text-truncate">{startCase(row.original.type)}</p></div> : <NoDataCell />
			},
			{
				accessor: 'detail',
				Header: 'Detail',
				...
				Cell: ({ row }) => (
					<div className="flex items-center gap-2">
						...
						<IconButton
							...
							aria-label="Details"
							onClick={() => {
								window.open(`${row.original.type === MATERIAL_TYPE.product
									? routes.productDetail.path : row.original.type === MATERIAL_TYPE.package ?
										routes.packagesDetail.path : routes.serializedAssetDetail.path}/${row.original.materialId}`);
							}}
						>
