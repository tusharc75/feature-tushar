...
const OnboardingTemplate = () => {
  const renderedFrom = camelCase(sidebarResource.onboardingTemplate);
  ...
  const fetchGridColumns = async () => {
    ...
    const newColumns = generateColumns(
      renderedFrom, 
      fieldsDataForRead, 
      routes.onboardingTemplateDetail.path, 
      true
    );
  ...
  const ActionsRenderer = {
    ...
    Cell: ({ row }) => (
      <>
        <HtmlTooltip title={permissions?.onboardingTemplate?.isUpdate ? 'Edit' : editDisable}>
          <span>
            <IconButton
              size="small"
              aria-label="Edit"
              disabled={!permissions?.onboardingTemplate?.isUpdate}
              onClick={() => {
                setShowManageDialog({ open: true, id: row.original._id, isClone: false });
              }}
            >
              <Edit fontSize="small" color={permissions?.onboardingTemplate?.isUpdate ? 'primary' : 'disabled'} />
            </IconButton>
          </span>
        </HtmlTooltip>
        <HtmlTooltip title={permissions?.onboardingTemplate?.isCreate ? 'Clone' : cloneDisable}>
          <span>
            <IconButton
              size="small"
              aria-label="Clone"
              disabled={!permissions?.onboardingTemplate?.isCreate}
              onClick={() => {
                setShowManageDialog({ open: true, id: row.original._id, isClone: true });
              }}
            >
              <FileCopy fontSize="small" color={permissions?.onboardingTemplate?.isCreate ? 'primary' : 'disabled'} />
            </IconButton>
          </span>
        </HtmlTooltip>
        <HtmlTooltip title={permissions?.onboardingTemplate?.isDelete ? 'Delete' : deleteDisable}>
          <span>
            <IconButton
              size="small"
              aria-label="Delete"
              disabled={!permissions?.onboardingTemplate?.isDelete}
              onClick={() => {
                setShowDeleteConfirmBox({ open: true, data: row.original });
              }}
            >
              <Delete fontSize="small" color={permissions?.onboardingTemplate?.isDelete ? 'error' : 'disabled'} />
            </IconButton>
          </span>
        </HtmlTooltip>
      </>
  ...
  const fetchData = async (cancelTokenSource?: CancelTokenSource) => {
    ...
    try {
      const response = await axiosInstance().get(`${routes.onboardingTemplate.path}${queryString}`, {
  ...
  const handleDelete = async (data = null) => {
    ...
    if (recordsToDelete.length > 0) {
      ...
      try {
        await axiosInstance().put(`${routes.onboardingTemplate.path}/remove`, {
          ids: recordsToDelete
        });
  ...
  return (
    <section className="main-container-v1">
      <div className="headerbox-v1">
        <CustomBreadCrumbs 
          routes={[{ ...routes.onboardingTemplate, title: resources?.onboardingTemplate?.titlePlural }]} 
        />
      </div>
      ...
      <CustomContainer>
        <ListingPageHeader
          ...
          isActionButtonVisible={permissions?.onboardingTemplate?.isDelete}
          ...
          isAddButtonVisible={permissions?.onboardingTemplate?.isCreate}
        />
        ...
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete ${
            showDeleteConfirmBox?.data 
              ? `${resources?.onboardingTemplate?.titleSingular?.toLowerCase()}: ${showDeleteConfirmBox?.data?.templateName}`
              : 'selected record(s)'
          }?`}
