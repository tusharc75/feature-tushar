...
const ManageTransferAsset = (
  ...
  useEffect(() => {
    axiosInstance().get("/field?resource=Transfer Asset").then(({ data: { data } }) => {
      const fieldsDataForCreate = data.filter((obj) => obj.isCreate)
        .map((d: any) => d.fieldData);
      const fieldsDataForUpdate = data.filter((obj) => obj.isUpdate).map((d: any) => d.fieldData);
      if (transferAssetId) {
        axiosInstance().get(`${transferAsset.api}/` + transferAssetId).then(({ data: { data } }) => {
          if (isClone) {
  ...
  const handleSubmit = (values) => {
    ...
    if (transferAssetId && isClone === false) {
      ...
      axiosInstance().put(`${transferAsset.api}`, values).then(({ data: { data } }) => {
        setSubmitting(false)
        onSuccess()
      }).catch((error) => {
        setSubmitting(false);
      ...
      axiosInstance().post(`${transferAsset.api}`, values).then(({ data: { data } }) => {
        setSubmitting(false);
        onSuccess(data)
      }).catch((error) => {
        setSubmitting(false);
    ...
    {initialData && initialData.fields.length ?
      ...
      >
        {({ values,
          ...
          <Fragment>
            <CustomDialogHeader title={transferAssetId ? (isClone ? "Clone" : "Update " + routes.transferAsset.title) : "Create " + routes.transferAsset.title}
              onClose={() => {
            ...
            <CustomDialogFooter>
              <Button size="small" color="primary" disabled={isSubmitting}
                onClick={() => {
                  if (isFieldNotTouched(initialData, values)) onClose()
                  else setShowConfirmDialog(true)
                }}
