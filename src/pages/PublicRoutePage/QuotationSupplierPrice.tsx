...
const QuotationSupplierPrice = ({ quotationData, openAuthId }) => {
    ...
    const fetchProduct = () => {
        ...
        axios.get(backendApi + `/quotation/supplier-price-request/supplier-price-response/${quotationData?.data?.requestId}`).then(({ data: { data } }) => {
    ...
    const onCellValueChanged = (row) => {
        ...
        if (productIndex === -1) {
            ...
            tempProductData[productIndex][`price_${quotationDetailsData?.currency.toLowerCase()}`] = tempData[`price_${quotationDetailsData?.currency.toLowerCase()}`]
            setProductData(tempProductData)
        }
    }
    ...
    const handleSubmit = () => {
        ...
        if (checkField) {
            ...
            axios.post(backendApi + `/quotation/supplier-price-request/supplier-price-response `, tempData).then(({ data }) => {
                setIsSubmited(true)
            }).catch((error) => {
                toastConfig.setToastConfig(error);
            });
