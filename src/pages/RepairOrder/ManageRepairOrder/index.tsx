...
const ManageRepairOrder = ({ isClone = false, repairOrderId = null, onClose, onSuccess, refrenceType = null, refrenceData = null }) => {
  ...
  useEffect(() => {
    setLoading(true);
    axiosInstance().get('/field?resource=Repair Order').then(({ data: { data } }) => {
      ...
      if (repairOrderId) {
        axiosInstance().get(`${repairOrder.api}/` + repairOrderId).then(({ data: { data } }) => {
          if (isClone) {
            const { _id, brand, createdBy, history, repairOrderNumber, updatedBy, ...rest } = data;
            setTitle(`Clone - ${repairOrderNumber}`)
            ...
            setTitle(`Editing - [${data.repairOrderNumber}]`)
            setInitialData({
        ...
        setTitle('Create Repair Order')
        setDisablePlantIfAssetAdded(false);
        let initialData = { ...getObjKeys('', fieldsDataForCreate), repairOrderNumber: `RO_${generateUniqueIdOnly()}` };
        setAllFields(fieldsDataForCreate);
        setInitialData({
          fields: setFieldsInAscendingOrder(fieldsDataForCreate),
          values: initialData
