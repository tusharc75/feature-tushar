...
const RepairOrderQtyDialog: FC<EditDialogProps> = (
  ...
  const fetchData = async () => {
    var { fields: data } = await fetch_rental_product_fields(repairOrderData?.currency, isOffline);
    ...
    packages.forEach((row) => {
      resetFields.forEach((ele) => {
        if (ele.type === "amount") {
          ...
          const cur = repairOrderData?.currency?.toLowerCase();
  ...
  const handleSubmit = async (values) => {
    const currency = repairOrderData?.currency?.toLowerCase();
    if (isBulkedit) {
                                    ...
                                    <FormTypes
                                      ...
                                      onChange={(e, val) => {
                                        ...
                                        getPricing({ ...values, [field.fieldName]: value }).then((price: any) => {
                                          if (price) {
                                            let priceFieldName = "price_" + repairOrderData?.currency?.toLowerCase()
                                  ...
                                  <Box display="flex" >
                                    <Box flexGrow={1}  >
                                      <FormTypes
                                        ...
                                        tooltipMessage={field.tooltipMessage}
                                        size="small"
                                        minDate={repairOrderData?.estimateStartDate}
                                        maxDate={repairOrderData?.estimateEndDate}
                                      />
                                    </Box>
                                  </Box>
                                </Grid>
