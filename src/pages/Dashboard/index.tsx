        ...
        <Grid className={styles.dashboard_layout}>
          ...
          <div>
            <Box marginY={2} id="resourcesHomeGrid">
              ...
              {
                search.trim() === "" ? <Grid container spacing={1} className={styles.full_layout}>
                  {
                ...
                </Grid> : filteredData.map(section => {
                  return <List key={section.head} subheader={<ListSubheader className={`${styles.list_header} mb-2`}>{section.head}</ListSubheader>}>
                    {
                      section.items.map(item => {
                        return <>
                          <ListItem
