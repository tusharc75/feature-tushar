...
import { getLocalStorageArrayData, gridLoadingTimeout, prepareDataForGrid, flash, sidebarResource } from '../../constants/helpers';
...
const Flash = () => {
  let renderedFrom = camelCase(routes.flash?.title);
  ...
  const fetchGridColumns = () => {
    axiosInstance()
      ...
      .then(({ data: { data } }) => {
        ...
        let rendererNames = [];
        data.forEach((o) => {
          let currentColumn = getColumnData(routes.flash?.title, o?.fieldData, routes.flashDetail.path, true);
    ...
    <section className="main-container-v1">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
            <div className={'d-flex flex-wrap align-items-center gap-1 w-full'}>
              <div className="d-flex align-items-center">
                ...
                <span className="listingHeader">{routes.flash?.title} </span>
              </div>
              {isMobile && !isTablet ? (
          ...
          Object.keys(frameWorkComponent).length > 0 ? (
            isMobile && !isTablet ? (
              <CustomSwipableList
                ...
                onClick={(data) => {
                  history.push(`${routes.flashDetail.path}/${data._id}`);
                }}
                dataRows={dataRows}
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete the ${routes.flash?.title?.toLowerCase()} ${deleteRecord?.flash || ''} ? `}
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
          }}
