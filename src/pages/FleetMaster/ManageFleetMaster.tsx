...
const ManageFleetMaster = ({ isClone = false, id = null, onClose, onSuccess, }) => {
    ...
    useEffect(() => {
        setLoading(true);
        axiosInstance().get('/field?resource=Fleet Master').then(({ data: { data } }) => {
    ...
    const handleSubmit = (values) => {
        handleUpdateFleetMaster(values);
    };

    const handleUpdateFleetMaster = (values) => {
        setSubmitting(true);
        if (id && isClone === false) {
            values._id = id;
            axiosInstance()
            ...
            axiosInstance().post(`${routes?.fleetMaster.path}`, values).then(({ data: { data, message } }) => {
                setSubmitting(false);
                onSuccess(data);
                toastConfig.setToastConfig({
                    open: true,
