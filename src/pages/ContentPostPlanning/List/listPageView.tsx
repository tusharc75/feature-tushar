...
import { sidebarResource, prepareDataForGrid } from 'src/constants/helpers';
...
const ListView = ({ topRightSlot }) => {
    ...
    useEffect(() => {
        fetchData();
    }, [search, page, limit, filters, sorting, showFilteredRecordsOnly, selectedType]);
    ...
    const fetchData = async () => {
        ...
        const response: any = await axiosInstance().get(`/content-post-planning${queryString}`)
            ...
            <CustomContainer>
                ...
                {showDeleteConfirmBox && (
                    <ConfirmationDialog
                        ...
                        message={`Are you sure you want to delete ${deleteRecord ? `${resources?.contentPostPlanning?.titleSingular?.toLowerCase()} : ${deleteRecord?.title || deleteRecord?.name}` : `selected ${resources?.contentPostPlanning?.titlePlural?.toLowerCase()}`
                            } ?`}
                        onClose={() => {
                            setDeleteRecord(null);
                            setShowDeleteConfirmBox(false);
