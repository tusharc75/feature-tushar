...
import {
    ...
    getObjKeysWithValues, getOwnerDropdownDataSource, isFieldNotTouched, invoice, setFieldsInAscendingOrder, yupSchema,
...
const ManageInvoiceDialog = ({ isClone, invoiceId, invoiceData= null, onClose, onSuccess, open }) => {
    ...
    useEffect(() => {
        const ownerCollabOptions = invoiceFormData.fields.filter(
        ...
        let customerAccountOptions = invoiceFormData.fields.find(
            (d) => d.fieldName === "customerAccount"
        );
        if (customerAccountOptions) {
            setAccountData(customerAccountOptions.option);
        }
        let customerContactOptions = invoiceFormData.fields.find(
            (d) => d.fieldName === "customerContact"
        );
        if (customerContactOptions) {
            setContactData(customerContactOptions.option);
        }
        const customerContactDropdownData = invoiceFormData.fields.find(
            (d) => d.fieldName === "customerContact"
        );
        const countryBillToDropdownData = invoiceFormData.fields.find(
        ...
        const countrySellToDropdownData = invoiceFormData.fields.find(
            ...
            if (invoiceId) {
                setCustomerContactDataSource(
                    customerContactDropdownData.option.filter(
                        (d) =>
                            d.parentAccount === invoiceData?.customerAccount.optionValue
                    )
                );
            }
        }
    ...
    const handleUpdateInvoice = (values) => {
        ...
        if (invoiceId && isClone === false) {
            ...
            axiosInstance().put(`${invoice.api}`, values).then(({ data }) => {
                setLoading(false);
                onSuccess()
            ...
            axiosInstance().post(`${invoice.api}`, values).then(({ data: { data, message } }) => {
                                                                            ...
                                                                            field.fieldName === "shippingAddress" ? (
                                                                                ...
                                                                                : (
                                                                                    <FormTypes
                                                                                        ...
                                                                                        fieldData={field}
                                                                                        disabled={
                                                                                            field.fieldName === "currency" ? (invoiceDetails && invoiceDetails?.material?.length ? true : false) :
                                                                                                field.fieldName === "warehouse" ? (invoiceDetails && invoiceDetails?.productInventory?.length ? true : false) :
