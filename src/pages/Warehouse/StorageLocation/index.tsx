...
const StorageLocation = ({ warehouse }) => {
  ...
  const fetchGridColumns = () => {
    axiosInstance()
      .get(`/field?resource=${sidebarResource.storageLocation}`)
      .then(({ data: { data } }) => {
        let columns = [];
        let rendererNames = [];
        data.forEach((o) => {
  ...
  const fetchData = () => {
    ...
    axiosInstance()
      .get(`${routes.storageLocation.path}${queryString}`)
      .then(({ data: { data } }) => {
  ...
  const getQueryString = () => {
    ...
    let filterById = [];
    filterById.push({ field: 'warehouse', term: warehouse });
  ...
  const handleDelete = () => {
    axiosInstance()
      .put(`${routes.storageLocation.path}/remove`, { ids: deleteRecord })
      .then(() => {
        fetchData();
    ...
    <Fragment>
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialogRaw
          ...
          message={`Are you sure you want to delete the ${routes.storageLocation?.title?.toLowerCase()} ?`}
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
          }}
