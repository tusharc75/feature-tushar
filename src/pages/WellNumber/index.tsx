...
const WellNumber = () => {
  ...
  const fetchData = async (cancelTokenSource?: CancelTokenSource) => {
    ...
    axiosInstance()
      .get(`${routes?.wellNumber.path}${queryString}`, { cancelToken: cancelTokenSource?.token })
    ...
    <section className="main-container-v1">
      ...
      <CustomContainer>
        <ListingPageHeader
          ...
          isActionButtonVisible={permissions?.wellNumber?.isDelete}
          actionButtonProps={{ disabled: selectedRecords?.length ? false : true }}
          actionMenuItems={<ActionMenuItems />}
          addButtonOnclick={() => {
            setShowManageDialog({ open: true, isClone: false, idToClone: null });
          }}
          isAddButtonVisible={permissions?.wellNumber?.isCreate}
        />
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete ${deleteRecord ? `${resources?.wellNumber?.titleSingular?.toLowerCase()} :
            ${deleteRecord?.wellNumber || ''}` : `selected ${resources?.wellNumber?.titlePlural?.toLowerCase()}`} ?`}
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
          }}
