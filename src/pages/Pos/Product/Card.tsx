...
const ProductCardLayout = ({ setAssignCartProductQty, plantId, searchVal, productCategory, refreshData }) => {
  ...
  const fetchMoreData = () => {
    setTimeout(() => {
      let api = searchVal
        ? `/pos?warehouse=${plantId}&page=${page}&limit=${limit}&search=${searchVal}`
        : `/pos?warehouse=${plantId}&page=${page}&limit=${limit}`;
      const filterById = [];
      if (productCategory && productCategory !== '') {
        filterById.push({ field: 'productCategory', term: productCategory });
      }
    ...
    <div className={classes.flexGrow1}>
      <div id="scrollableDiv" style={{ height: '80vh', overflowY: 'auto' }}>
        <InfiniteScroll
          ...
          hasMore={hasMore}
          loader={<h4 className="loading-dots mt-3 border p-3 text-center">Loading more product(s)</h4>}
