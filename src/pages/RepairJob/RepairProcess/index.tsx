...
const RepairProcess = ({ onClose, onSuccess, assetId, assetNumber, repaired, repairJobData }) => {
    ...
    const fetchRepairProcess = () => {
        axiosInstance().get(`${repairJob.api}/${repairJobData?._id}/repair-process/${assetId}`)
            .then(({ data: { data } }) => {
    ...
    const handleUpdate = (_id, name, status) => {
        axiosInstance().post(`${repairJob.api}/${repairJobData?._id}/repair-process/${assetId}`, { _id: _id, name: name, status: status })
            .then(({ data: { data } }) => {
    ...
    const handleCompleteRepair = () => {
        setLoading(true)
        axiosInstance().put(`${repairJob.api}/${repairJobData._id}/assets/repaired`,
            { assets: [assetId], repaired: true }).then(({ data }) => {
    ...
    >
        <Fragment>
            <CustomDialogHeader title={`Repair Process - ${assetNumber}`}
                onClose={() => {
                    onClose()
                }}
                    ...
                    {repairProcess?.map((process: any, i) => (
                        <Fragment>
                            ...
                            {process?.steps?.map((step, index) => (
                                <Box key={index} bgcolor="white" p={2} borderTop={1} borderColor="grey.300" width={'100%'}>
                                    <Grid container >
                                        <Grid item xs={1} sm={1} md={1} lg={1}>
            ...
            <CustomDialogFooter>
                {(allowComplete && !repaired) && <CustomButton
                    loading={loading}
                    variant="contained"
                    color="primary"
                    type="submit"
