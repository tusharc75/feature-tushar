...
import { CustomDialogTransition, serviceOrder, customerAccount, supplierAccount } from "src/constants/helpers";
...
const SendEmail = ({ serviceOrderData }) => {
    ...
    const fetchEmailsData = () => {
        ...
        if (serviceOrderData?.collaborator && serviceOrderData.collaborator.length) {
            ownerCollaboratorEmails = serviceOrderData.collaborator.filter((o) => o?.email).map((o) => o?.email);
        }
        if (serviceOrderData?.owner?.email) {
            ownerCollaboratorEmails.push(serviceOrderData.owner.email);
        }
        let toEmails = [];
        if (serviceOrderData?.supplier?.email) {
            toEmails.push(serviceOrderData.supplier.email);
        }
    ...
    if (pdfFileBase64) {
        attachments.push({
            ...
            name: `Service Order-${serviceOrderData.serviceOrderNumber}`
        });
    }

    const fetchEmailAttachment = () => {
        axiosInstance().get(`/pdf/${serviceOrderData._id}?resource=Service Order`)
            .then(({ data }) => {
                axiosInstance()
                ...
                axiosInstance()
                    ...
                    .then(({ data }) => {
                        ...
                        if (download) {
                            ...
                            link.href = url;
                            link.setAttribute('download', `ServiceOrder-${serviceOrderData.serviceOrderNumber}.pdf`);
    ...
    const handleAttachments = () => {
        ...
        request = {
            ...
            relatedTo: [
                {
                    type: serviceOrder.resource,
                    referenceId: serviceOrderData?._id,
                    access: true
                },
                {
                    type: serviceOrderData?.customerAccountName ? customerAccount?.accountResource : supplierAccount?.accountResource,
                    referenceId: serviceOrderData?.customerAccountName
                        ? serviceOrderData?.customerAccountName?.optionValue
                        : serviceOrderData?.supplierAccountName?.optionValue,
                    access: false
                }
            ]
        };
        ...
        <Box display="flex" justifyContent="space-between">
            <Box display="flex" alignItems="center">
                <Box display="flex">
                    ...
                    <Box mx={1} />
                    {permissions?.serviceOrder?.isRead && <Button
                        variant="outlined"
                        color="primary"
                        size="small"
        ...
        {sendEmail && (
            ...
            >
                <CreateEmail
                    ...
                    subject={`${user?.user?.brandName ?? 'Brand'} Offer - ${serviceOrderData?.serviceOrderId ?? ''}`}
                    fromQuote={true}
