    ...
    if (bulkEdit) {
      ...
      data.forEach((element) => {
        ...
        if (element.fieldName === 'competence') {
          element.option = technicianData?.technician?.competencies || [];
        }
      });
      setInitialData({
        fields: data,
  ...
  async function getAllPricingCondition(values: any, pricingMethodOptions: any) {
    if (technicianData) {
      ...
      if (user?.user?.brandPolicy?.materialCostPrice) {
        let costPriceData: any = await getCostPriceConditions([{ materialId: values.competence, technician: technicianData.technicianId }], ['competency', 'technician'], rentalManagementData);
                                      ...
                                      <Box display="flex">
                                        <Box flexGrow={1}>
                                          <FormTypes
                                            ...
                                            options={
                                              field.fieldName === 'pricingCondition' ? priceConditionList
                                                : field.fieldName === 'pricingMethod' ? priceMethodList
                                                  : field.option
                                            }
                                            onChange={(e, val) => {
