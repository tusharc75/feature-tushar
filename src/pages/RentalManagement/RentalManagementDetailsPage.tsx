...
const RentalManagementDetailsPage = () => {
  ...
  const calculatePricing = (arr: any[]) => {
    ...
    if (rentalManagementData) {
      ...
      return new Promise((resolve, reject) => {
        axiosInstance()
          .post(pricingConditionApi + `/calculatePrice`, data)
          .then(({ data: { data } }) => {
            resolve(data);
          })
          .catch((err) => {
  ...
  const fetchRentalManagementData = async () => {
    try {
      ...
      setAdditionalCost(
        data?.additionalCost?.map((d, index) => {
          return { id: d?._id, rowIndex: index + 1, type: d.type, amount: d?.value, description: d?.description, uom: d.uom, qty: d.qty };
        }) ?? []
      );
        ...
        childrens.forEach((current, index) => {
          if (current.parent === parent[childProperty]) {
            ...
            if (
              ...
              current?.finalPrice !== undefined &&
              typeof current?.finalPrice !== 'string' &&
              current?.finalPrice !== 0
            ) {
  ...
  const fetchProductInventory = () => {
    ...
    axiosInstance()
      .get(`${rentalManagement.rentalManagementApi}/${id}/products-packages`)
      .then(({ data }) => {
        data.data?.products.map((u: any, index) =>
          tempInventory.push({
            ...u,
            id: u._id,
  ...
  const columns = [
    {
      ...
      Cell: ({ row }) => (
        <div style={{ width: 250, display: 'flex', alignItems: 'center' }}>
          <p onClick={() => handleClick(row.original)} className="link text-truncate" title={row.original.detail}>
            {row.original.detail}
          </p>
    ...
    // },
    {
      ...
      Cell: ({ row }) =>
        row.original.startDate ? (
          <h5 className="createBy text-truncate" title={`${moment(row.original.startDate.slice(0, 10)).format(dateFormat)}`}>
    ...
    },
    {
      ...
      Cell: ({ row }) =>
        row.original.endDate ? (
          <h5 className="createBy text-truncate" title={`${moment(row.original.endDate.slice(0, 10)).format(dateFormat)}`}>
    ...
    },
    {
      ...
      Header: 'Quantity',
      Cell: ({ row }) => (row.original.qtyToDisplay ? <p>{row.original.qtyToDisplay}</p> : <NoDataCell />)
    },
    {
      accessor: 'UOM',
      Header: 'UOM',
      Cell: ({ row }) => (row.original.UOM ? <p>{startCase(row.original.UOM)}</p> : <NoDataCell />)
    },
    {
      accessor: 'pricingMethod',
      Header: 'Pricing Method',
      Cell: ({ row }) => (row.original.pricingMethod ? <p>{startCase(row.original.pricingMethod)}</p> : <NoDataCell />)
    },
    {
      accessor: 'price',
      Header: `Price (${currencySymbol})`,
      Cell: ({ row }) => (row.original.price ? <p>{row.original.price}</p> : <NoDataCell />),
      Footer: (info) => {
    ...
    },
    {
      accessor: 'discount',
      Header: 'Discount (%)',
      Cell: ({ row }) => (row.original.discount ? <p>{row.original.discount}</p> : <NoDataCell />)
    },
    {
      accessor: 'finalPrice',
      Header: `Final Price (${currencySymbol})`,
      Cell: ({ row }) => (row.original.finalPrice ? <p>{row.original.finalPrice}</p> : <NoDataCell />),
      Footer: (info) => {
        const total = React.useMemo(
          () =>
            info.rows
              .filter((f) => f.values.hasOwnProperty('finalPrice') && !isNaN(f.values.finalPrice))
              .reduce((sum, row) => row.values.finalPrice + sum, 0),
          [info.rows]
        );
  ...
  const handleAddProductInventory = (productInventoryArray) => {
    ...
    axiosInstance()
      .post(`${rentalManagement.rentalManagementApi}/${id}/products-packages`, { productsPackages: tempProductArray })
      .then(() => {
        setAddExistingProductDialog({ open: false, type: '' });
        fetchProductInventory();
  ...
  const handleRemoveProductInventory = (productInventoryId) => {
    setDeleting(true);
    axiosInstance()
      .put(`${rentalManagement.rentalManagementApi}/${id}/products-packages/remove`, {
  ...
  const updateProductData = (data) => {
    ...
    axiosInstance()
      .put(`${rentalManagement.rentalManagementApi}/${id}/products-packages`, { productsPackages: updatedArr })
      .then(() => {
        fetchProductInventory();
      })
      .catch((error) => {
  ...
  const handleSingleEdit = async (values: any) => {
    ...
    axiosInstance()
      .put(`${rentalManagement.rentalManagementApi}/${id}/products-packages/updateOne`, rest)
      .then(() => {
        setUpdating(false);
        setIsProductEdit({ open: false, editType: null });
        fetchProductInventory();
  ...
  return (
    <>
                ...
                <Paper>
                    ...
                    <>
                      <Box display="flex" justifyContent="space-between" m={1}>
                        ...
                        <Box display="flex">
                          ...
                          <Box mx={1} />
                          <HtmlTooltip
                            title={Boolean(selectedProducts && selectedProducts.length) ? 'Delete selected records' : 'Select records to delete'}
                          >
                            <span>
                              <Button
                                variant="contained"
                  ...
                  {currentStep === 2 && (
                    <SerializedAssetStep
                      ...
                      productInventory={[...productInventory, ...serializeAssets]}
      ...
      {showManageAdditionalCostDialog.open && (
        <ManageAdditionalCostDialog
      ...
      {additionalCostDeleteConfirmation.open && (
        <ConfirmationDialog
