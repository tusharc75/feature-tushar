...
const PackageDialog = ({ onClose, rentalManagementData, onSuccess }) => {
	...
	const fetchData = () => {
		axiosInstance().get(`${rentalManagement.api}/productpackage/${rentalManagementData._id}`)
			.then(({ data: { data } }) => {
				setPackageOptions(data?.material?.filter(m => m?.type === MATERIAL_TYPE.package)?.map(m => ({ optionLabel: m?.packageDetail?.packageName, optionValue: m?._id })))
			}).catch((error) => {
