...
const TransferToAnotherProductDialog = ({ onClose, onSuccess, rentalManagementData, assets, assetPolicyData }) => {
  ...
  const fetchData = async () => {
    ...
    axiosInstance()
      .patch(`${rentalManagement.api}/productpackage/${rentalManagementData._id}/package-similar-product`, data)
      .then(({ data: { data } }) => {
  ...
  const handleAdd = (rows) => {
    ...
    axiosInstance()
      .post(`${rentalManagement.api}/productpackage/${rentalManagementData._id}/move-asset-inter-package-product`, data)
      .then(({ data }) => {
  ...
  return (
    <>
      ...
      >
        <Box>
          <CustomDialogHeader onClose={onClose} title={'Transfer to another Package'} showManimizeMaximize={false} showRequiredLabel={false} />
          <CustomDialogContent>
            <Box>
      ...
      {openAssetDataDialog.open && (
        <AssetDetailsChangeDialog
          ...
          productsDefaultData={selectedRecords?.map((d) => ({
            materialId: d?.materialId,
            assetDefaultData: [{ wellNumber: d?.wellNumber, qty: d?.qty, package: d?.packageId }]
          }))}
        />
      )}
    </>
