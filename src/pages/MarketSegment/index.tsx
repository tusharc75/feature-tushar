...
const MarketSegment = () => {
  ...
  const fetchData = async (cancelTokenSource?: CancelTokenSource) => {
    ...
    axiosInstance()
      .get(`${routes?.marketSegment.path}${queryString}`, { cancelToken: cancelTokenSource?.token })
    ...
    <section className="main-container-v1">
      ...
      <CustomContainer>
        <ListingPageHeader
          ...
          isActionButtonVisible={permissions?.marketSegment?.isDelete}
          actionButtonProps={{ disabled: selectedRecords?.length ? false : true }}
          actionMenuItems={<ActionMenuItems />}
          addButtonOnclick={() => {
            setShowManageDialog({ open: true, isClone: false, idToClone: null });
          }}
          isAddButtonVisible={permissions?.marketSegment?.isCreate}
        />
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete ${deleteRecord ? `${resources?.marketSegment?.titleSingular?.toLowerCase()} : ${deleteRecord?.name || ''}` : `selected ${resources?.marketSegment?.titlePlural?.toLowerCase()}`} ?`}              
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
          }}
