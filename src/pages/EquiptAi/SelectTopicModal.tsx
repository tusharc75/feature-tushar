    ...
    >
      ...
      <div className="mb-2 flex flex-wrap gap-2">
        ...
        {selectedTopics?.map((c) => (
          <Chip color="primary" variant="outlined" size="small" label={c.aiModelTopicName} onDelete={() => handleUnselect(c)} />
        ))}
      </div>
      <div className="flex-grow overflow-auto rounded-md p-3 [border:1px_solid_var(--common-border-color)] ">
        <div className="flex flex-grow flex-wrap gap-2 overflow-auto ">
          {filteredTopics.map((topic) => {
            return <Chip variant="outlined" size="small" label={topic.aiModelTopicName} onClick={() => handleSelect(topic)} clickable />;
          })}
        </div>
      </div>
    </DashboardModal>
