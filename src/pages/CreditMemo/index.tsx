...
const CreditMemo = () => {
  ...
  const fetchGridColumns = () => {
    axiosInstance()
      ...
      .then(({ data: { data } }) => {
        ...
        let rendererNames = [];
        data.forEach((o) => {
          let currentColumn = getColumnData(renderedFrom, o?.fieldData, routes.creditMemoDetail.path, true);
    ...
    axiosInstance()
      ...
      .then(({ data: { data } }) => {
        ...
        let rows = data?.data?.map((u: any) => {
          ...
          finalObject['canDelete'] = permissions?.creditMemo?.isDelete;
  ...
  const ActionsRenderer = (params) => (
    <Fragment>
      {permissions?.creditMemo?.isCreate ? (
        <Tooltip title="Clone">
          <IconButton
            aria-label="Clone"
            onClick={() => {
              setCreditMemoId(params.data.id);
  ...
  return (
    <section className="main-container-v1">
      <div className="headerbox-v1">
        <CustomBreadCrumbs routes={[{ title: routes.creditMemo.title }]} />
        <ImportExportLinks
      ...
      <CustomContainer>
        ...
        {Object.keys(frameWorkComponent).length > 0 ? (
          isMobile && !isTablet ? (
            ...
            <CustomAgGrid
              ...
              loading={loading}
              renderedFrom={renderedFrom}
              refreshGrid={fetchCreditMemoData}
        ...
        {showDeleteConfirmBox && (
          <ConfirmationDialog
            ...
            message={`Are you sure you want to delete ${routes?.creditMemo?.title?.toLowerCase()}  ${deleteRecord?.creditMemoNumber || ''} ?`}
            onClose={() => {
              setDeleteRecord(null);
              setShowDeleteConfirmBox(false);
            }}
