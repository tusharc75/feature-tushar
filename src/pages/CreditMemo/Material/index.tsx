...
const Material = ({ creditMemoData, allowedToEdit }) => {
  ...
  const fetchFields = async () => {
    let data = await fetch_child_resource_fields(CHILD_RESOURCE.invoiceProduct, creditMemoData?.currency, allowedToEdit);
    let childFields = await fetch_child_resource_fields(CHILD_RESOURCE.invoiceCost, creditMemoData?.currency, allowedToEdit);
    setCostFields(childFields);
    setAllFields(JSON.parse(JSON.stringify(data)));
    const newColumns = generateColumns(renderedFrom, data, null, false, creditMemoData?.currency);
  ...
  const fetchData = async () => {
    ...
    var data: any = [];
    let assignedAssets = [];
    const response = await axiosInstance().get(`${routes?.creditMemo.path}/material/${creditMemoData._id}`);
    const additionalData = await axiosInstance().get(`${routes?.creditMemo.path}/${creditMemoData._id}/additional-cost`);
    ...
    rows.forEach((parent, i) => {
      ...
      parent.isValid = parent['finalPrice_' + creditMemoData?.currency?.toLowerCase()] ? true : !isRateRequired;
  ...
  const generateNestedData = (material, parent) => {
    ...
    subRows.forEach((_subRow, index) => {
      ...
      _subRow.isValid = _subRow['finalPrice_' + creditMemoData?.currency?.toLowerCase()] ? true : false;
    });
    return subRows;
  };
  ...
  const handleAddCost = (rows) => {
    setUpdating(true);
    axiosInstance()
      .post(`${routes.creditMemo?.path}/${creditMemoData?._id}/additional-cost`, [rows])
      .then(({ data }) => {
        setUpdating(false);
        fetchData();
  ...
  const handleSaveCostData = async (rows: any, saveAndNext = false) => {
    setUpdating(true);
    axiosInstance()
      .put(`${routes.creditMemo?.path}/${creditMemoData?._id}/additional-cost`, [rows])
      .then(({ data }) => {
        setUpdating(false);
        toastConfig.setToastConfig({
          open: true,
  ...
  const handleDelete = (rows) => {
    ...
    if (products?.length) {
      axiosInstance()
        .put(`${routes.creditMemo?.path}/material/${creditMemoData?._id}/delete`, { ids: products })
        .then(() => {
          setDeleting(false);
          fetchData();
          setDeleteData(null);
    ...
    if (cost?.length) {
      axiosInstance()
        .put(`${routes?.creditMemo?.path}/${creditMemoData._id}/additional-cost/remove`, { ids: cost })
        .then(({ data }) => {
          fetchData();
          setDeleteData(null);
          toastConfig.setToastConfig({
  ...
  const onSaveInlineEdit = async (inputField, updatedData) => {
    ...
    if (rowData?.type === MATERIAL_TYPE.manualEntry) {
      rows = await calculateRowsField(flattenArray(dataRows), inputField, costFields, updatedData, creditMemoData?.currency);
      handleSaveCostData(rows[0]);
    } else {
      rows = await calculateRowsField(flattenArray(dataRows), inputField, allFields, updatedData, creditMemoData?.currency);
      handleSaveData(rows);
    }
  };
