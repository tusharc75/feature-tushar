...
const MaterialDialog: FC<EditDialogProps> = ({
  ...
  const fetchFields = async () => {
    setLoading(true);
    var data = await fetch_child_resource_fields(CHILD_RESOURCE.invoiceProduct, creditMemoData?.currency, true);
  ...
  const EvaluteproductFields = async (fields) => {
    ...
    if (
      creditMemoData?.taxCode ||
      (creditMemoData?.billingAddress &&
        (creditMemoData?.billingAddress?.zipCode || creditMemoData?.billingAddress?.state || creditMemoData?.billingAddress?.county))
    ) {
      const taxCodeOptions = await fetchTaxRate(creditMemoData?.billingAddress, creditMemoData?.taxCode?.optionValue || null);
  ...
  const handleSubmit = async (values) => {
    if (isBulkedit) {
      const rows = bulkUpdate(values, selectedProducts, material, allFields, creditMemoData?.currency);
      handleSaveData(rows);
    } else {
      ...
      if (rowData.parentId && !showConfirmationDialog) {
        ...
        const rows = await calculateRowsField(material, values, allFields, rowData, creditMemoData?.currency);
                                      ...
                                      <Box flexGrow={1}>
                                        <FormTypes
                                          ...
                                          onChange={(e, val) => {
                                            ...
                                            let priceFieldName = 'price_' + creditMemoData?.currency?.toLowerCase();
