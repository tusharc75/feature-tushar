...
const ManageWorkStations = ({ onClose, onSuccess, isClone = false, id = null }) => {
  ...
  const fetchFields = async () => {
    try {
      ...
      if (id) {
        axiosInstance()
          .get(`${routes?.workStations?.path}/${id}`)
          .then(({ data: { data } }: any) => {
  ...
  const handleSubmit = (values) => {
    ...
    if (id && !isClone) {
      values._id = id;
      axiosInstance()
        .put(`${routes.workStations?.path}`, values)
        .then(({ data }: any) => {
          setSubmitting(false);
          onSuccess();
      ...
      axiosInstance()
        .post(`${routes.workStations?.path}`, values)
        .then(({ data: { data, message } }: any) => {
          setLoading(false);
          history.push(`${routes.workStationsDetail.path}/${data._id}`);
          onSuccess(data);
          setSubmitting(true);
          toastConfig.setToastConfig({
            open: true,
