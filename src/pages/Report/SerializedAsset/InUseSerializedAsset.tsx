...
const InUseSerializedAssetReport = () => {
    ...
    const resource = sidebarResource.serializedAsset;
    let resourceCamelCase = camelCase(resource);
    let resourceStartCase = startCase(resource);
    const renderedFrom = `In-Use_${resource}_report`;
    ...
    const fetchGridColumns = async () => {
        ...
        const resourceColumns = [...fieldData];
    ...
    const RentalJobRenderer = (params) => params.value ? (
        <>
            <Link
    ...
    const AddressRenderer = (params) => params.value ? (
        <>
            <Link
                className="link text-truncate"
                to={`${routes.addressDetail.path}/${params?.data?.rentalJobId}`}
                target={'_blank'}
                rel="noopener noreferrer"
            >
                {params?.value}
    ...
    const fetchResourceData = () => {
        ...
        let api = `${routes[resourceCamelCase].path}/report/in-use-assets/${filterQuery}`;
    ...
    const exportData = () => {
        ...
        let api = `${routes[resourceCamelCase].path}/report/in-use-assets/export${filterQuery}`
                        ...
                        <Grid item xs={10}>
                            <CustomBreadCrumbs
                                routes={[
                                    ...
                                    { title: `In-Use Serialized Assets`, path: '' }
                                ]}
                            />
                        </Grid>
                        <Grid item xs={2}>
...
export default InUseSerializedAssetReport;
