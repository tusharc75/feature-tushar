...
const CostBooks = ({ refresh = null }) => {
  ...
  useEffect(() => {
    ...
    if (costBooksTimeout) {
      clearTimeout(costBooksTimeout);
    }
    costBooksTimeout = setTimeout(() => {
      fetchData();
    }, millisec);
  }, [search]);
  ...
  const ActionsRenderer = {
    ...
    Cell: ({ row }) => (
      <>
        <HtmlTooltip title={permissions?.costBooks?.isCreate ? 'Clone' : cloneDisable}>
          <span>
            <IconButton
              size="small"
              aria-label="Clone"
              disabled={permissions?.costBooks?.isCreate ? false : true}
              onClick={() => {
                setShowManageTicketsDialog({ open: true, isClone: true, idToClone: row.original._id });
              }}
            >
              <FileCopyIcon fontSize="small" color={permissions?.costBooks?.isCreate ? 'primary' : 'disabled'} />
            </IconButton>
          </span>
        </HtmlTooltip>
  ...
  const fetchData = async (cancelTokenSource?: CancelTokenSource) => {
    ...
    try {
      ...
      let rows = data.map((u) => {
        ...
        finalObject['canDelete'] = permissions?.costBooks?.isDelete;
        return finalObject;
      });
  ...
  return (
    <div className="main-container-v1">
      <div className="headerbox-v1">
        <CustomBreadCrumbs routes={[{ ...routes.costBooks, title: resources?.costBooks?.titlePlural }]} />
        <ImportExportLinks
          permissions={permissions.costBooks}
          module={resources?.costBooks?.titlePlural}
          api={costBooks.api}
      ...
      <CustomContainer>
        <ListingPageHeader
          ...
          isAddButtonVisible={permissions?.costBooks?.isCreate}
        />
        {columns ? (
          <CustomReactTable
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete ${
            deleteRecord
              ? `${resources?.costBooks?.titleSingular?.toLowerCase()} : ${deleteRecord?.costBookName}`
              : `selected ${resources?.costBooks?.titlePlural?.toLowerCase()}`
          } ?`}
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
      ...
      {showManageTicketsDialog.open && (
        <ManageCostBooks
          ...
          costBookId={showManageTicketsDialog.idToClone}
