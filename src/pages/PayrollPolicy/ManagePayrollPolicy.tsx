...
const ManagePayrollPolicy = ({ onClose, onSuccess, isClone = false, id = null }) => {
  ...
  const fetchFields = async () => {
    try {
      ...
      if (id) {
        axiosInstance()
          .get(`${routes?.payrollPolicy?.path}/${id}`)
          .then(({ data: { data } }) => {
            let fields = fieldsDataForUpdate;
            let tempData = data;
            if (isClone) {
              fields = fieldsDataForCreate;
              const { payrollPolicyName, ...rest } = data;
              setCloneHeading(payrollPolicyName);
              tempData = rest;
            }
            setInitialData({
              fields: fields,
  ...
  const handleSubmit = (values) => {
    ...
    if (id && !isClone) {
      values._id = id;
      axiosInstance()
        .put(`${routes.payrollPolicy?.path}`, values)
        .then(({ data }) => {
          setSubmitting(false);
          onSuccess();
      ...
      axiosInstance()
        .post(`${routes.payrollPolicy?.path}`, values)
        .then(({ data }) => {
          setLoading(false);
          setSubmitting(true);
          history.push(`${routes.payrollPolicyDetail.path}/${data?.data?._id}`);
          toastConfig.setToastConfig({
            open: true,
            type: 'success',
            message: data.message
