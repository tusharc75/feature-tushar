...
function QtyDialog({ open, loading, onClose, data = null, logData, onSuccess }) {
  function validate(values) {
    ...
    if (data && logData?.qty) {
      ...
      if (parseInt(values.qty) > parseInt(logData?.qty || 0)) {
        errors['qty'] = 'You can revert only Processed qty!';
      }
    }
    return errors;
  }
  ...
  return (
    <Dialog open={open} fullWidth TransitionComponent={CustomDialogTransition}>
      <CustomDialogHeader onClose={onClose} title={`${data.productName}`} showManimizeMaximize={false} showRequiredLabel={false} />
      <Formik
        initialValues={{ qty: parseInt(logData?.qty || 0) }}
        onSubmit={(values, { setSubmitting }) => {
          values.product = data?.product?.optionValue;
