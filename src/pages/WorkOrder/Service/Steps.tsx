...
const Steps = ({
  ...
  const getFields = (step) => {
    ...
    const productData =
      selectedService?.parentId && products?.some((p) => p?._id === selectedService?.parentId && p?.productDetail)
        ? products?.find((p) => p?._id === selectedService?.parentId)?.productDetail
        : null;
    ...
    if (tempServiceData) {
      ...
      if (step?.fields?.length) {
        ...
        fieldData = {
          ...
          values: isDataAlreadyAdded
            ? getObjKeysWithValues(tempServiceData, fieldsDataForCreate, true)
                ...
                return (
                  <>
                    <CustomCollapsible
                      ...
                      toggleIconPosition={
                        workOrderPolicyData?.policy?.hideStepsProductsConsumables && workOrderPolicyData?.policy?.hideStepsDrawings ? 'hidden' : 'end'
                      }
                      head={
                        <Box
                          key={step._id}
                        ...
                        >
                          <div className="flex gap-2 ">
                            ...
                            <span className="rounded-full bg-[var(--primary)] px-[12px] py-[1px] text-[13px] text-white dark:bg-[var(--dark-primary)]">
                              {resource === sidebarResource.workOrderTechnician
                                ? step?.order
                                : `${selectedService?.order}.${step?.order || index + 1}`}
                            </span>
                            <div
                                    ...
                                    (stepData?.status === WORKORDER_SERVICE_STEP_STATUS.start && !user?.brandPolicy?.workOrderTimer ? null : (
                                      <ThemeButton
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          if (
                                            [WORKORDER_SERVICE_STEP_STATUS.pause, WORKORDER_SERVICE_STEP_STATUS.needReperform].includes(
                                              stepData?.status
                                            )
                                          ) {
