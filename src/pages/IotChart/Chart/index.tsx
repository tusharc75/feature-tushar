...
function Index({ serializedAssets = [], type }) {
  ...
  const fetchIotDataPointsData = async () => {
    setLoading(true);
    const data: any = await axiosInstance().get(`/report/iot/data-points${serializedAssets?.length > 0 ? `? filterById=${JSON.stringify([{
      field: 'asset', term: {
        "$in": serializedAssets
      }
    }])}
              ...
              <div className="flex flex-col overflow-auto gap-2">
                {
                  type === 'dataPoint' ? (
                    ...
                    Object.keys(dataVal).map((particularCategory) => (
                      <Accordion expanded={expandedAccordition === particularCategory} onChange={() => {
                        setExpandedAccordition(particularCategory)
                      }}>
                        <AccordionSummary aria-controls="panel1d-content" id="particularCategory">
                          <Typography style={{ fontSize: '14.2056px', fontWeight: 600 }}>{particularCategory}
                            <IconButton size="small">{expandedAccordition === particularCategory ? <ExpandLessIcon /> : <ExpandMoreIcon />}</IconButton></Typography>
                        </AccordionSummary>
                        <AccordionDetails>
