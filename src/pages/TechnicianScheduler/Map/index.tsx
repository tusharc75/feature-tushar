...
const MapView = ({ technician, onClose }) => {
  ...
  const fetchData = async () => {
    try {
      const { data: { data } } = await axiosInstance().get(`/technician-scheduler/technician-service/${technician}`);
      if (data) {
        setServiceData(data);
      }
    } catch (error) {
  ...
  return (<Box width={'100%'} height={'100%'} overflow="hidden" borderRadius={1}>
    <GoogleMap
      options={{
      ...
      <MarkerClusterer>
        {(clusterer) => (
          <>
            ...
            {selectedService && (
              <InfoWindow
                position={{ lat: selectedService?.shippingAddress?.latitude, lng: selectedService?.shippingAddress?.longitude }}
