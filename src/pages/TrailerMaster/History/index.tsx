...
const renderedFrom = `${camelCase(routes?.trailerMaster.title)}_History`;
...
const History = ({ id, status }) => {
        ...
        <>
          {row?.original?.reference ? (
            <p
              ...
              onClick={() => window.open(`${routes.trailerMasterDetail.path}/${row?.original?.reference?.optionValue}`)}
            >
  ...
  const fetchData = () => {
    ...
    axiosInstance()
      .get(`${routes.trailerMaster?.path}/history/${id}`)
      .then(({ data: { data } }) => {
