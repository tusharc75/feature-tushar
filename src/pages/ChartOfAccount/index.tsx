...
const ChartOfAccount = () => {
  ...
  const fetchGridColumns = async () => {
    let data;
    const response = await axiosInstance().get(`/field?resource=${sidebarResource.chartOfAccount}`);
    data = response?.data?.data;
    const newColumns = generateColumns(renderedFrom, data, routes.chartOfAccountDetail.path, true);
  ...
  const ActionsRenderer = {
    ...
    Cell: ({ row }) => (
      <>
        {permissions?.chartOfAccount?.isCreate ? (
  ...
  const fetchData = async (cancelTokenSource?: CancelTokenSource) => {
    ...
    axiosInstance()
      .get(`${routes?.chartOfAccount.path}${queryString}`, { cancelToken: cancelTokenSource?.token })
      .then(({ data: { data } }) => {
        let count = data?.count;
        let rows = data?.data?.map((u) => {
          let finalObject = prepareDataForGrid(u, user);
          finalObject['isChecked'] = selectedRecords?.some((s) => s._id === u._id);
          finalObject['allowedToEdit'] = permissions?.chartOfAccount?.isUpdate;
          finalObject['canDelete'] = permissions?.chartOfAccount?.isDelete;
          return finalObject;
        });
  ...
  const handleDelete = () => {
    ...
    axiosInstance()
      .put(`${routes?.chartOfAccount.path}/remove`, { ids: ids })
      .then(() => {
    ...
    <section className="main-container-v1">
      ...
      <CustomContainer>
        <ListingPageHeader
          ...
          isActionButtonVisible={permissions?.chartOfAccount?.isDelete}
          actionButtonProps={{ disabled: selectedRecords?.length ? false : true }}
          actionMenuItems={<ActionMenuItems />}
          addButtonOnclick={() => {
            setShowManageDialog({ open: true, isClone: false, idToClone: null });
          }}
          isAddButtonVisible={permissions?.chartOfAccount?.isCreate}
        />
      ...
      {showDeleteConfirmBox && (
        <ConfirmationDialog
          ...
          message={`Are you sure you want to delete ${deleteRecord ? `${resources?.chartOfAccount?.titleSingular?.toLowerCase()} :
            ${deleteRecord?.accountNumber || ''}` : resources?.chartOfAccount?.titlePlural?.toLowerCase()} ?`}
          onClose={() => {
            setDeleteRecord(null);
            setShowDeleteConfirmBox(false);
          }}
