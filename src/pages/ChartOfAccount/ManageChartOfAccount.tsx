...
const ManageChartOfAccount = ({ onClose, onSuccess, isClone = false, id = null }) => {
  ...
  const handleSubmit = (values) => {
    ...
    if (id && !isClone) {
      values._id = id;
      axiosInstance()
        .put(`${routes.chartOfAccount?.path}`, values)
        .then(({ data }: any) => {
          setSubmitting(false);
          onSuccess();
      ...
      axiosInstance()
        .post(`${routes.chartOfAccount?.path}`, values)
        .then(({ data: { data, message } }: any) => {
          setLoading(false);
          history.push(`${routes.chartOfAccountDetail.path}/${data._id}`);
          onSuccess(data);
          setSubmitting(true);
          toastConfig.setToastConfig({
            open: true,
              ...
              <CustomDialogHeader
                ...
                title={`${
                  id
                    ? isClone
                      ? `Clone - ${cloneHeading}`
                      : `Update ${initialData.values?.accountNumber ? `(${initialData.values?.accountNumber})` : ''}`
                    : `Create ${resources?.chartOfAccount?.titleSingular}`
                }`}
