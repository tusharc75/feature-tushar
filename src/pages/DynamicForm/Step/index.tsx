    ...
    <>
      {steps && steps?.length ? (
        tab?.stepsStyle === STEPS_STYLE.step ? (
          ...
          <>
            <div
              className={`relative flex min-h-[calc(100vh-280px)] [--collapsed-width:0px] [--sidebar-w:min(100%,300px)] md:min-h-[calc(100vh-200px)] md:gap-4 md:[--collapsed-width:100px]`}
            >
              {isSidebarVisible() && (
                <div
                  className={cn(
                    'bottom-0 left-0 top-0 flex-shrink-0 overflow-hidden bg-[var(--dark-primary,white)] transition-all duration-300 max-md:absolute max-md:z-10',
              ...
              <div className={`container-with-border flex-grow overflow-y-auto overflow-x-hidden p-[20px]`}>
                <View
                  sidebarButton={
                    <HtmlTooltip className="md:!hidden" title={isExpanded ? 'Collapse Sidebar' : 'Expand Sidebar'}>
                      <IconButton size="small" onClick={() => setIsExpanded((prev) => !prev)}>
                        <KeyboardArrowLeft
                          className="transition-all duration-300 data-[expanded=false]:[transform:rotate(180deg)]"
                          data-expanded={isExpanded}
                ...
                {isExpanded && (
                  <div
                    className="absolute inset-0 cursor-pointer bg-white/10 [backdrop-filter:blur(2px)] dark:bg-black/10 md:hidden"
