...
export default function AccountHierarchy({
  ...
  const columns = [
      ...
      render: (rowData: any) => (
        <div style={{ width: 250 }}>
          {rowData._id === currentAccountId ? (
            ...
            <Link
              className="link"
              to={`/${accountRoute}/detail/${rowData._id}`}
            >
              {rowData.accountName}
            </Link>
          )}
          {rowData._id === currentAccountId ? (
            <Chip label="Current" size="small" className="ml-2" />
          ) : (
            ""
          )}
        </div>
    ...
    },
    {
      ...
      field: "typeOfAccount",
      render: (rowData: any) => (
        <div style={{ width: commonFieldWidth }}>{rowData.typeOfAccount}</div>
      ),
    },
    {
      title: "Industry",
      field: "industry",
      render: (rowData: any) => (
        <div style={{ width: commonFieldWidth }}>{rowData.industry}</div>
      ),
    },
    {
      ...
      render: (rowData: any) => (
        <div style={{ width: commonFieldWidth }}>{rowData.typeOfBusiness}</div>
      ),
    },
    {
      ...
      render: (rowData) => (
        <div style={{ width: commonFieldWidth }}>
          {rowData.parentAccountId === currentAccountId ? (
            ...
            <Link
              className="link"
              to={`/${accountRoute}/detail/${rowData.parentAccountId}`}
            >
        ...
        <Box margin={10}>
          <MaterialTable
            ...
            parentChildData={(row, rows) =>
              rows.find((a) => a._id === row.parentAccountId)
            }
            options={options}
          />
        </Box>
