      ...
      if (visibilityCondition?.flatMap((item) => item?.fields.map((field) => field?.fieldName))?.includes(name)) {
        ...
        for (let i = 0; i < visibilityCondition?.length; i++) {
          ...
          if (condition?.logic === LOGIC.AND) {
            if (!condition?.fields?.every((f) => f?.fieldName === name && checkValue(fields, f?.fieldName, value, f?.value))) {
              visible = false;
            }
          } else if (condition?.logic === LOGIC.OR) {
            if (!condition?.fields?.some((f) => f?.fieldName === name && checkValue(fields, f?.fieldName, value, f?.value))) {
              visible = false;
            }
          }
          if (!visible) {
    ...
    if (fieldData.type === 'dropDown' || fieldData.type === 'multiSelect') {
      fields &&
        fields
          .filter(
            (_f: any) =>
              (_f.type === 'dropDown' || _f.type === 'multiSelect') && _f.isDependentDropdown && _f.dropdowDependentOn === fieldData.fieldName
          )
          .forEach((_r: any) => {
  ...
  fieldData.formulaFields.forEach((_field: any) => {
    if (resultValues[_field] === undefined) {
      ...
      let calValue = getFormulaValue(
        fieldData.formulaoption[_field],
        formulainputFields,
        'decimal',
        fieldData.decimalPlaces ? fieldData.decimalPlaces : 2
      );
      ...
      if (_field_result.length) {
        if (_field_result[0].type !== 'currencyAmount' && (_field_result[0].type === 'converter' || _field_result[0].isConverter === true)) {
          ...
          resultValues = handleCurrencyConverter(
            ...
            fields,
            values,
            _field_result[0].fieldName,
            _field.split('_')[1].toUpperCase(),
            _field.split('_')[2],
            calValue,
            resultValues
          );
          ...
          if (_data.type === 'currencyAmount' || _data.type === 'converter' || _data.isConverter) {
            if (_data.type !== 'currencyAmount' && (_data.type === 'converter' || _data.isConverter === true)) {
              _fieldName =
                _fieldName +
                '_' +
                (_data.formulaOnConverter && _data.formulaOnConverter !== ''
                  ? _data.formulaOnConverter.toLowerCase()
                  : _data.displayUnits[0].toLowerCase());
            } else if (_data.type === 'currencyAmount' && (_data.type === 'converter' || _data.isConverter === true)) {
              _fieldName =
                _fieldName +
                '_' +
                (_data.formulaOnCurrency && _data.formulaOnCurrency !== ''
                  ? _data.formulaOnCurrency.toLowerCase()
                  : _data.displayCurrency[0].toLowerCase()) +
                '_' +
                (_data.formulaOnConverter && _data.formulaOnConverter !== ''
                  ? _data.formulaOnConverter.toLowerCase()
                  : _data.displayUnits[0].toLowerCase());
...
const handleCheckVlookupReverse = (fieldData, fields, values, name, value, resultValues) => {
  if (fields && fields.filter((_f: any) => (_f.type === 'vlookupDropdown' || _f.isVlookup) && !_f.isvlookupReverse).length) {
    fields
      .filter((_f: any) => (_f.type === 'vlookupDropdown' || _f.isVlookup) && !_f.isvlookupReverse)
      .forEach((_data: any) => {
        if (_data.vlookupInputFields.includes(name)) {
          let result =
            _data.option &&
            _data.option.filter(function (val: any) {
          ...
          if (result.length) {
            if (_data.type === 'currencyAmount' || _data.type === 'converter' || _data.isConverter) {
              if (_data.type !== 'currencyAmount' && (_data.type === 'converter' || _data.isConverter === true)) {
                ...
                resultValues = handleFormula(
                  _data,
                  fields,
                  values,
                  _data.fieldName + '_' + _unit.toLowerCase(),
                  result[0].optionLabel,
                  resultValues,
                  true
                );
              } else if (_data.type === 'currencyAmount' && (_data.type === 'converter' || _data.isConverter === true)) {
                const _unit = _data.vlookupOnConverter ? _data.vlookupOnConverter : _data.displayUnits[0];
                resultValues[_data.fieldName + '_' + _data.displayCurrency[0].toLowerCase() + '_' + _unit.toLowerCase()] = parseFloat(
                  result[0].optionLabel
                );
                resultValues = handleCurrencyConverter(
                  _data,
                ...
                resultValues = handleFormula(
                  ...
                  fields,
                  values,
                  _data.fieldName + '_' + _data.displayCurrency[0].toLowerCase() + '_' + _unit.toLowerCase(),
                  result[0].optionLabel,
                  resultValues,
                  true
                );
                ...
                resultValues = handleFormula(
                  ...
                  fields,
                  values,
                  _data.fieldName + '_' + _data.displayCurrency[0].toLowerCase(),
                  result[0].optionLabel,
                  resultValues,
                  true
                );
...
const handleCurrencyConverter = (fieldData, fields, values, name, _currency, _unit, value, resultValues) => {
  ...
  if (indexConverter >= 0 && indexCurrency >= 0 && fieldData.unitoption) {
    for (var x_unit in fieldData.unitoption[indexConverter]) {
      for (var x_currency in fieldData.currencyoption[indexCurrency]) {
        if (
          (fieldData.displayUnits.includes(x_unit) || (fieldData.formulaUnits && fieldData.formulaUnits.includes(x_unit))) &&
  ...
  fieldList.forEach((ele: any) => {
    ...
    if (ele.lookup || ele.isUneditable) {
      ...
      if (ele.type === 'converter' || ele.type === 'currencyAmount' || ele.isConverter === true) {
        if (ele.type !== 'currencyAmount' && (ele.type === 'converter' || ele.isConverter === true)) {
          ele.displayUnits &&
            ele.displayUnits.forEach((_unit: any) => {
              const fieldName = ele.fieldName + '_' + _unit.toLowerCase();
              if (values[fieldName] && values[fieldName] !== '') {
                calValues = handleAutoCalculation(ele, fieldList, returnvalues, fieldName, '', _unit, values[fieldName] ? values[fieldName] : 0);
              }
            });
        } else if (ele.type === 'currencyAmount' && (ele.type === 'converter' || ele.isConverter === true)) {
          ele.displayCurrency &&
            ele.displayCurrency.forEach((_currency: any) => {
              ele.displayUnits &&
                ele.displayUnits.forEach((_unit: any) => {
          ...
          ele.displayCurrency &&
            ele.displayCurrency.forEach((_currency: any) => {
              ...
              if (values[fieldName] && values[fieldName] !== '') {
                calValues = handleAutoCalculation(ele, fieldList, returnvalues, fieldName, _currency, '', values[fieldName] ? values[fieldName] : 0);
              }
            });
        }
      } else {
...
export const autoCalculateSpecificFields = (inputValues: any, values: any, fieldList: any) => {
  ...
  for (var _fieldName in inputValues) {
    ...
    if (field.length) {
      ...
      const calValues = handleAutoCalculation(
        fieldData,
        fieldList,
        { ...values, ...returnvalues },
        _fieldName,
      ...
      if (checkfromFields.filter((_d) => _d.fieldLabel.trim().toLowerCase() === _f.fieldLabel.trim().toLowerCase()).length > 0) {
        ...
        if (checkfromFields.filter((_d) => _d.fieldLabel.trim().toLowerCase() === _f.fieldLabel.trim().toLowerCase())[0].templateName) {
          error_field =
            error_field +
            ' (Template - ' +
            checkfromFields.filter((_d) => _d.fieldLabel.trim().toLowerCase() === _f.fieldLabel.trim().toLowerCase())[0].templateName +
            ')';
        }
        same_type = 'Label';
        is_same = true;
...
export const checkFieldDependency = (fieldId, sectionId, section) => {
  try {
    ...
    if (fieldData.type === 'converter' || fieldData.type === 'currencyAmount' || fieldData.isConverter === true) {
      if (fieldData.type !== 'currencyAmount' && (fieldData.type === 'converter' || fieldData.isConverter === true)) {
        fieldData.formulaUnits &&
          fieldData.formulaUnits.forEach((_unit: any) => {
            fieldNames.push(fieldData.fieldName + '_' + _unit.toLowerCase());
          });
      } else if (fieldData.type === 'currencyAmount' && (fieldData.type === 'converter' || fieldData.isConverter === true)) {
        fieldData.displayCurrency &&
          fieldData.displayCurrency.forEach((_currency: any) => {
            fieldData.formulaUnits &&
              fieldData.formulaUnits.forEach((_unit: any) => {
...
export const updateFieldAccordingCurrency = (fields: any, oldFieldName: any, newFieldName: any, oldCurrency: any, currency: any) => {
  ...
  fields.forEach((_f: any) => {
    ...
    if (_f.formulaoption) {
      for (const x in _f.formulaoption) {
        if (x === oldFieldName) {
          _f.formulaoption[newFieldName] = _f.formulaoption[x].replace(regexOldFieldName, newFieldName);
    ...
    if (_c.displayCurrency.length && _c.displayCurrency[0] === 'CUR') {
      if (_c.isConverter && _c.formulaUnits) {
        _c.formulaUnits.forEach((_unit: any) => {
          const oldFieldName = _c.fieldName + '_' + _c.displayCurrency[0].toLowerCase() + '_' + _unit.toLowerCase();
          const newFieldName = _c.fieldName + '_' + currency.toLowerCase() + '_' + _unit.toLowerCase();
          if (oldFieldName !== newFieldName) {
            fields = updateFieldAccordingCurrency(fields, oldFieldName, newFieldName, _c.displayCurrency[0], currency);
          }
        });
