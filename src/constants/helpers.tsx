...
const isFieldValidateEntityWise = (fieldData, selectedEntity) => {
  ...
  fields.forEach((input) => {
    ...
    if (input.type === 'singleLine') {
      schema[input.fieldName] =
        input.required && isFieldValidateEntityWise(input, selectedEntity)
      ...
      schema[input.fieldName] =
        input.required && validEmail && isFieldValidateEntityWise(input, selectedEntity)
          ? validationFields?.length && validation
            ? string().when(
                validationFields?.map((f) => f?.fieldName),
                {
